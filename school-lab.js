...
function renderModules(){
 ...
 const counts=new Map();st.devices.forEach(d=>{if(!d.online)return;const n=window.ZebjusDroneKit.normalizeKitName(d.deviceName||'');if(n)counts.set(n,(counts.get(n)||0)+1)});
 box.innerHTML=list.map(d=>{const sel=d.deviceId===st.selectedDeviceId,linked=sel&&selectedConnected(),dup=(counts.get(window.ZebjusDroneKit.normalizeKitName(d.deviceName||''))||0)>1,ctl=linked?(d.lockMine?'YOU CONTROL':'CONNECTED'):d.lockMine?'YOU CONTROL':d.locked?'VIEW ONLY':'AVAILABLE',dupBadge=dup?'<span class="duplicate-kit-name">DUPLICATE NAME</span>':'';return `<button class="module-card ${sel?'selected':''}" data-device="${esc(d.deviceId)}"><span class="module-online-dot ${d.online?'on':''}"></span><div><b>${esc(d.deviceName||d.deviceId)} ${dupBadge}</b><small>${esc(d.deviceId)}</small><em>${esc(d.ip||'IP pending')} • ${esc(d.ssid||'Same Wi-Fi')} • ${Number.isFinite(+d.rssi)?d.rssi+' dBm / '+signalText(d.rssi):'RSSI --'}</em></div><i class="${linked||d.lockMine?'mine':d.locked?'busy':''}">${d.online?ctl:'OFFLINE'}</i></button>`}).join('');
...
function renderSelected(){
 ...
 const net=$('#settingsNetworkSummary');if(net)net.textContent=selectedConnected()?`STA: ${d.ssid||'--'}  •  IP: ${d.ip||'--'}  •  ${Number.isFinite(+d.rssi)?d.rssi+' dBm / '+signalText(d.rssi):'RSSI --'}  •  ${d.hostname||d.deviceName||''}.local  •  ${d.deviceId}`:'Connect a verified kit to view its Wi-Fi and saved networks.';
 const flight=$('#settingsFlightState');if(flight)flight.textContent=selectedConnected()?`${d.deviceName}  •  ${d.armed?'ARMED':'DISARMED'}  •  ${d.flightMode||'ANGLE'}  •  ${d.rcSource||'NONE'}  •  ${d.lockMine?'YOU CONTROL':d.locked?'VIEW ONLY':'AVAILABLE'}`:`Simulator  •  ${modeName()}  •  ${st.txOn?'TRANSMITTER ON':'TRANSMITTER OFF'}`;
...
async function connectExact(query,autoAcquire=true){
 ...
 // A typed Device ID is strict. A typed Kit Name is live-discovered and may rebind to a
...
async function requestModules(force=false){
 if(!ready()||st.reconnectBusy)return;const now=performance.now(),interval=selectedConnected()?30000:15000;if(!force&&now-st.lastDiscoverAt<interval)return;st.lastDiscoverAt=now;
 const extras=[st.preferredDeviceName,st.query].filter(x=>x&&!/^ZJ-DRONE-/i.test(x));
 try{const found=await window.ZebjusDroneKit.scanDefaultKits({max:30,extraNames:extras}),seen=new Set();found.forEach(r=>{const d=upsertStatus(r.status,r.base);if(d)seen.add(String(d.deviceId).toUpperCase())});const nowMs=Date.now();st.devices.forEach(d=>{if(d.deviceId===st.selectedDeviceId&&selectedConnected())return;if(!seen.has(String(d.deviceId).toUpperCase())&&nowMs-(d.lastSeen||0)>45000){d.online=false;d.lockMine=false;d.lastSeenText='Not seen in recent scans'}});reconcileSelection();clearError();try{statusUi()}catch(e){console.warn('[ZEBJUS] discovery UI update skipped',e)}}catch(e){console.warn('[ZEBJUS] Background discovery',e)}
}
...
async function i2cScan(){
 ...
 catch(e){if(e?.status===404)throw new Error('I2C scan API is unavailable on this kit. Install the matching current FlightCore firmware.');throw e}
}
...
const READ_ONLY_DEVICE_COMMANDS=new Set(['ping','pid_get','receiver_read','ppm_read','attitude_read','calibration_get','bench_status']);
