...
async function healthRefresh(force=false){
 ...
 st.healthBusy=true;
 try{
   const s=await client.refresh(1200);
   ...
   if(current&&st.failures<FAILURE_LIMIT&&age<OFFLINE_AFTER_MS){current.lastSeenText='Checking link…';renderSelected();}
   if(st.failures>=FAILURE_LIMIT||age>=OFFLINE_AFTER_MS){log(`Kit offline • ${e?.message||'health check failed'}`);markSelectedOffline('Connection lost');}
...
async function telemetryTick(now){if(!client?.connected||now-st.lastTelemetryAt<150)return;st.lastTelemetryAt=now;try{const t=await client.telemetry();st.lastTelemetryGoodAt=Date.now();healthFromTelemetry(t);const rcLive=(t?.receiverHealth||t?.sensorHealth?.receiver)==='OK'||(Array.isArray(t?.rc)&&(+t.rcAgeMs||0)<700);if(rcLive)applyPhysicalReceiver(t);else if(st.receiverHealth==='STALE'){api()?.controlSim?.({roll:0,pitch:0,yaw:0});api()?.setSimRunning?.(false)}api()?.receiveDevicePacket(t);api()?.setFcConnected(true);updateHealthUi()}catch(_){updateHealthUi()}}
...
function start(){if(st.booted)return;st.booted=true;initUi();requestAnimationFrame(loop);setInterval(()=>{refreshLastSeenText();healthRefresh();reconnectTick();requestModules(false)},1000);document.addEventListener('visibilitychange',()=>{if(!document.hidden){healthRefresh(true);reconnectTick(true)}});window.addEventListener('online',()=>{healthRefresh(true);reconnectTick(true)});window.zebjusSchool={sendDeviceCommand,isViewOnly,isCloudActive:()=>!!selected()?.online,isKitActive:()=>!!selected()?.online,getSelectedDevice:selected,canControl,ownsLock,requestModules,acquireLock,releaseLock,state:st,client,markOffline:markSelectedOffline,refreshNow:async()=>{await healthRefresh(true);return selected()},reconnectNow:async()=>{const d=selected();if(!d)return null;if(d.online&&client?.connected){await healthRefresh(true);return selected()}if(d.online)markSelectedOffline('Reconnect requested');await reconnectTick(true);return selected()}}}
