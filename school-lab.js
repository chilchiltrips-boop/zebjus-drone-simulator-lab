...
async function scanKitsUi(){
 const btn=$('#refreshModulesBtn'),sel=$('#kitSelect');if(btn)btn.disabled=true;if(sel)sel.innerHTML='<option value="">Scanning…</option>';
 try{await requestModules(true);const list=st.devices.filter(d=>d.online);if(sel){sel.innerHTML='';if(!list.length)sel.innerHTML='<option value="">No default kits found</option>';else list.forEach(d=>{const o=document.createElement('option');o.value=d.deviceName;o.dataset.ip=d.ip||'';o.dataset.id=d.deviceId||'';o.textContent=`${d.deviceName} — ${d.ip||'local'}`;sel.appendChild(o)})}setText('kitNameMessage',list.length?`${list.length} kit(s) found on this Wi-Fi.`:'No default kit found. Enter a custom Kit Name once; it will be remembered after connection.');
