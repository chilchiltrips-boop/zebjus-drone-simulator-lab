...
1. Compile and flash the correct **A2** application image. Confirm firmware `18.3.45`, board `ZFC-A2`, detected `MPU6050`, and `flightReady=true` in Kit Status. LSM6DS3 at `0x6B` can be read for student projects; it is **not** an A2 flight-control IMU in this release.
...
## RC and loop measurement

The firmware now reports `ppmFrameHz`, `webRcFrameHz` and `flightLoopHz` as rolling one-second measurements. Read from `/api/telemetry`, the AP `/fly` page, Joystick page or Python `await drone.status()`. A 250 Hz loop is a target; the displayed number is observed and may fall during blocking HTTP and Wi-Fi activity. Record the minimum loop Hz, maximum gap, overrun count and measured source input while testing with propellers removed. Do not interpret a 50 Hz RC input as a 250 Hz flight-control loop or accept a nominal 250 Hz target as proof of scheduler stability.
