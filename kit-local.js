...
async function connect(query,ipHint='',expectedDeviceId='',clientId=''){
  ...
  const expected=String(expectedDeviceId||(/^ZJ-DRONE-/i.test(query)?query:'')||known?.deviceId||'');
  const name=/^ZJ-DRONE-/i.test(query)?(known?.name||''):query;
  const bases=[];
  ...
  for(const base of [...new Set(bases.filter(Boolean))]){
    try{
      ...
      if(st.kit!=='ZEBJUS_F450')throw new Error('This device is not a ZEBJUS F450 kit.');
      if(expected&&String(st.deviceId)!==expected)throw new Error('Device ID mismatch: this address belongs to another kit.');
...
class LocalKitClient{
  ...
  async refresh(){if(!this.base)throw new Error('Kit not connected.');const st=await requestBase(this.base,`/api/status?clientId=${encodeURIComponent(this.clientId)}`,{timeout:2100});if(this.deviceId&&String(st.deviceId)!==String(this.deviceId))throw new Error('Connected device identity changed.');return this._accept(st,this.base)}
