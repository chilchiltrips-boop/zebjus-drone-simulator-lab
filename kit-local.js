...
class LocalKitClient{
  ...
  async refresh(timeout=2100){if(!this.base)throw new Error('Kit not connected.');const st=await requestBase(this.base,`/api/status?clientId=${encodeURIComponent(this.clientId)}`,{timeout});if(this.deviceId&&String(st.deviceId)!==String(this.deviceId))throw new Error('Connected device identity changed.');return this._accept(st,this.base)}
