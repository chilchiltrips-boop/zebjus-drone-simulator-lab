/*
  ZEBJUS FlightCore V18.3.23 - LOCAL Wi-Fi / mDNS + I2C PYTHON LAB
...
// ---------------- General ----------------
static const char* FW_VERSION="18.3.23";
...
String statusJson(const String& clientId=""){
  ...
  String j="{\"ok\":true,\"kit\":\"ZEBJUS_FLIGHTCORE\",\"version\":\""+String(FW_VERSION)+"\",\"firmware\":\""+String(FW_VERSION)+"\"";
  ...
  j+=",\"mode\":\""+mode+"\",\"armed\":"+String(effectiveArmed()?"true":"false")+",\"locked\":"+String(lockActive()?"true":"false")+",\"lockMine\":"+String(lockMine(clientId)?"true":"false")+",\"lockTimeoutMs\":"+String(LOCK_TIMEOUT_MS)+",\"benchRc\":"+String(ALLOW_BENCH_RC?"true":"false")+",\"flightCoreIntegrated\":false,\"otaUpdate\":true,\"receiverHealth\":\""+receiverHealth()+"\",\"receiverPin\":"+String(PPM_RECEIVER_PIN)+",\"i2cScan\":true,\"i2cSda\":"+String(I2C_SDA_PIN)+",\"i2cScl\":"+String(I2C_SCL_PIN)+"}";
  return j;
}
...
void telemetryApi(){
  ...
  String j="{\"type\":\"telemetry\",\"roll\":0.0,\"pitch\":0.0,\"yaw\":0.0,\"gyroX\":0.0,\"gyroY\":0.0,\"gyroZ\":0.0,\"battery\":0.0,\"armed\":"+String(effectiveArmed()?"true":"false");
  ...
  // This network bridge does not fake sensor detection. Real FC integrations can replace these fields with live driver health.
  j+=",\"imuHealth\":\"NOT_FOUND\",\"barometerHealth\":\"NOT_FOUND\",\"lidarHealth\":\"NOT_FOUND\"";
  j+=",\"sensorHealth\":{\"imu\":\"NOT_FOUND\",\"barometer\":\"NOT_FOUND\",\"lidar\":\"NOT_FOUND\",\"receiver\":\""+rx+"\"}}";sendJson(200,j);
}
...
void firmwareInfoApi(){
  String j="{\"ok\":true,\"product\":\"ZEBJUS_FLIGHTCORE\",\"firmware\":\""+String(FW_VERSION)+"\",\"boardId\":\""+String(BOARD_ID)+"\",\"boardName\":\""+String(BOARD_NAME)+"\",\"flashBytes\":"+String(ESP.getFlashChipSize())+",\"freeSketchBytes\":"+String(ESP.getFreeSketchSpace())+",\"ota\":true,\"flightCoreIntegrated\":false,\"i2cScan\":true,\"i2cSda\":"+String(I2C_SDA_PIN)+",\"i2cScl\":"+String(I2C_SCL_PIN)+",\"armed\":"+String(effectiveArmed()?"true":"false")+"}";sendJson(200,j);
}
void rebootApi(){
...
void setupRoutes(){
  ...
  server.on("/api/status",HTTP_GET,statusApi);server.on("/api/telemetry",HTTP_GET,telemetryApi);server.on("/api/i2c/scan",HTTP_GET,i2cScanApi);
...
void startNormalServer(){
  ...
  Serial.println("==============================");Serial.println("ZEBJUS FlightCore V18.3.23 LOCAL MODE");Serial.println("Controller: "+String(BOARD_NAME)+" ["+String(BOARD_ID)+"]");Serial.println("Device ID: "+deviceId);Serial.println("Kit Name : "+kitName);Serial.println("SSID     : "+WiFi.SSID());Serial.println("IP       : "+WiFi.localIP().toString());Serial.println("mDNS     : http://"+hostFromName(kitName)+".local");
}
void startSetupMode(){
...
void setup(){
  ...
  Serial.println("\n==============================\nZEBJUS FlightCore V18.3.23 LOCAL Wi-Fi + I2C\nBoard: "+String(BOARD_NAME)+" ["+String(BOARD_ID)+"]\nID: "+deviceId+"\n==============================");
