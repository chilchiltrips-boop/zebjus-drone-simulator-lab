/*
  ZEBJUS FlightCore V18.3.43 - RATE/ANGLE FLIGHT CORE + PYTHON CONTROL LAB
...
// ---------------- General ----------------
static const char* FW_VERSION="18.3.43";
...
void commandApi(){
  ...
  if(type=="calibration_defaults"){if(effectiveArmed()){sendMessage(423,"Calibration restore blocked while armed.");return;}setCalibrationDefaults();saveCalibrationSettings();kalmanRollUnc=kalmanPitchUnc=4;sendJson(200,"{\"ok\":true,\"type\":\"ack\",\"command\":\"calibration_defaults\",\"calibration\":"+calibrationJson()+"}");return;}
  if(type=="level_calibrate"||type=="calibrate_level"){
    ...
    int requested=server.hasArg("samples")?server.arg("samples").toInt():400;int samples=constrain(requested,100,1000),good=0;float sx=0,sy=0,sz=0,sgx=0,sgy=0,sgz=0;for(int i=0;i<samples;i++){float rr,rp,ry,ax,ay,az;if(readMpuFlight(rr,rp,ry,ax,ay,az)){sx+=ax-accelOffsetX;sy+=ay-accelOffsetY;sz+=az-accelOffsetZ;sgx+=rr;sgy+=rp;sgz+=ry;good++;}delay(2);}if(good<(int)(samples*.9f)){sendMessage(500,"Level calibration failed: unstable MPU6050 reads.");return;}float axo=-(sx/good),ayo=-(sy/good),azo=1.0f-(sz/good);if(!calibrationValid(axo,ayo,azo,levelTrimRoll,levelTrimPitch)){sendMessage(400,"Calculated level offsets are outside safe limits. Check mounting/orientation.");return;}accelOffsetX=axo;accelOffsetY=ayo;accelOffsetZ=azo;gyroBiasRoll=sgx/good;gyroBiasPitch=sgy/good;gyroBiasYaw=sgz/good;saveCalibrationSettings();float rr,rp,ry;if(readMpuFlight(rr,rp,ry,accX,accY,accZ)){accAngleRoll=atan2f(accY,sqrtf(accX*accX+accZ*accZ))*57.2957795f+levelTrimRoll;accAnglePitch=-atan2f(accX,sqrtf(accY*accY+accZ*accZ))*57.2957795f+levelTrimPitch;kalmanRoll=accAngleRoll;kalmanPitch=accAnglePitch;kalmanRollUnc=kalmanPitchUnc=4;}resetFlightPid();sendJson(200,"{\"ok\":true,\"type\":\"ack\",\"command\":\"level_calibrate\",\"samples\":"+String(good)+",\"calibration\":"+calibrationJson()+",\"message\":\"Level accelerometer offsets and gyro bias captured\"}");return;
  }
...
void startNormalServer(){
  ...
  Serial.println("==============================");Serial.println("ZEBJUS FlightCore V18.3.43 LOCAL MODE");Serial.println("Controller: "+String(BOARD_NAME)+" ["+String(BOARD_ID)+"]");Serial.println("Device ID: "+deviceId);Serial.println("Kit Name : "+kitName);Serial.println("SSID     : "+WiFi.SSID());Serial.println("IP       : "+WiFi.localIP().toString());Serial.println("mDNS     : http://"+hostFromName(kitName)+".local");
}
void startSetupMode(){
...
void setup(){
  ...
  Serial.println("\n==============================\nZEBJUS FlightCore V18.3.43 LOCAL Wi-Fi + I2C\nBoard: "+String(BOARD_NAME)+" ["+String(BOARD_ID)+"]\nID: "+deviceId+"\n==============================");
