...
setBootStatus('V18.3.39 local-kit module loaded • starting engineering runtime…');
...
function registerOffline(){
 ...
 navigator.serviceWorker.register('./service-worker.js?v=18.3.39',{updateViaCache:'none'}).then(reg=>reg.update().catch(()=>{})).catch(()=>{});
}
...
function exportPythonProject(){pythonFiles[pythonActiveFile]=getPythonCode();const payload={schema:1,product:'ZEBJUS Python Lab',version:'18.3.39',exportedAt:new Date().toISOString(),active:pythonActiveFile,files:pythonFiles};downloadBlob('ZEBJUS_Python_Project.json',JSON.stringify(payload,null,2),'application/json')}
...
async function boot(){
 ...
 window.dispatchEvent(new CustomEvent('zebjus-app-ready',{detail:{three:threeOK,version:'18.3.39',assets}}));
}
