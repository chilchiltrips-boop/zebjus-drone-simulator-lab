...
setBootStatus('V18.3.40 local-kit module loaded • starting engineering runtime…');
...
function projectPayload(){return{version:'18.3.40',savedAt:new Date().toISOString(),guided:state.guided,step:state.step,parts:state.parts.filter(p=>!p.internal&&p.type!=='batteryStrap').map(p=>({type:p.type,slotId:p.slotId})),actions:[...state.doneActions],connections:state.connections,pid:state.pid,wireLayout,wireNodeTransforms,optionalWireNodes,sim:{batteryV:state.sim.batteryV,payloadG:state.sim.payloadG,cgX:state.sim.cgX,cgY:state.sim.cgY,wind:state.sim.wind,motorLag:state.sim.motorLag}}}
...
function registerOffline(){
 ...
 navigator.serviceWorker.register('./service-worker.js?v=18.3.40',{updateViaCache:'none'}).then(reg=>reg.update().catch(()=>{})).catch(()=>{});
}
...
function packet(raw){let d,text;if(typeof raw==='string'){text=raw;try{d=JSON.parse(raw)}catch{d={raw}}}else{d=raw||{};text=JSON.stringify(d)};['roll','pitch','yaw','gyroX','gyroY','gyroZ','battery'].forEach(k=>{if(Number.isFinite(+d[k]))state.telemetry[k]=+d[k]});if(Number.isFinite(+d.accX)&&Number.isFinite(+d.gyroX))recordImuSample?.(d);if(d?.type==='ack'&&d?.command==='pid_set'){pidDirty=false;updatePidSaveState?.('saved')}$('#telemetryLog').textContent=(new Date().toLocaleTimeString()+' '+text+'\n'+$('#telemetryLog').textContent).slice(0,12000);telemetryUI()}
...
let py=null,pyodidePromise=null,pythonMonaco=null,monacoPromise=null,pythonPreludeReady=false,pythonStopRequested=false,pythonRunning=false,pythonStderrBuffer='',pythonHasRun=false,pythonWorker=null,pythonWorkerRunId=0;
...
const PY_EXAMPLES=[
 ...
 {id:'imu',title:'IMU Sensor Live',desc:'Stream auto-detected LSM6DS3 / MPU6050 accelerometer + gyroscope values',code:PY_IMU_EXAMPLE,target:'real'}
];
...
function exportPythonProject(){pythonFiles[pythonActiveFile]=getPythonCode();const payload={schema:1,product:'ZEBJUS Python Lab',version:'18.3.40',exportedAt:new Date().toISOString(),active:pythonActiveFile,files:pythonFiles};downloadBlob('ZEBJUS_Python_Project.json',JSON.stringify(payload,null,2),'application/json')}
...
function pythonTerminalWrite(text){const term=$('#pythonTerminal');if(!term)return;term.textContent+=String(text||'');if(term.textContent.length>48000)term.textContent='… older terminal output trimmed …\n'+term.textContent.slice(-44000);term.scrollTop=term.scrollHeight}
...
function pythonHardwarePreflight(code){
 const target=$('#pythonTarget')?.value||'sim',needs=pythonHardwareNeeded(code);if(!needs)return true;
 if(target!=='real'){const term=$('#pythonTerminal');if(term){term.textContent='[TARGET] This program uses physical I²C/IMU APIs.\nChoose “Real ZEBJUS kit”, connect the selected kit, then press Run.\n';term.scrollTop=term.scrollHeight}$('#pyStatus').textContent='Real hardware target required';return false}
 if(!window.zebjusSchool?.isKitActive?.()){const term=$('#pythonTerminal');if(term){term.textContent='[HARDWARE] The selected ZEBJUS kit is not VERIFIED CONNECTED.\nOpen Kit Connect → connect the selected Device ID → return to Python Lab → press Run.\n';term.scrollTop=term.scrollHeight}$('#pyStatus').textContent='Verified kit connection required';notify('Connect the selected ZEBJUS kit first.','bad');return false}
 return true
}
...
async function pythonWorkerRpc(worker,m,target){try{let value=null;if(m.method==='attitude')value=JSON.stringify({roll:state.telemetry.roll,pitch:state.telemetry.pitch,yaw:state.telemetry.yaw});else if(m.method==='status')value=JSON.stringify(window.zebjusSchool?.getSelectedDevice?.()||{});else if(m.method==='i2cScan'){if(target!=='real')throw new Error('Hardware I²C is unavailable while Python target is Simulator.');if(!window.zebjusSchool?.isKitActive?.())throw new Error('Connect the selected ZEBJUS kit first.');value=JSON.stringify(await window.zebjusSchool.i2cScan())}else if(m.method==='imuRead'){if(target!=='real')throw new Error('Hardware IMU is unavailable while Python target is Simulator.');if(!window.zebjusSchool?.isKitActive?.())throw new Error('Connect the selected ZEBJUS kit first.');const d=await window.zebjusSchool.imuRead();recordImuSample(d);value=JSON.stringify(d)}else if(m.method==='command'){let data={};try{data=JSON.parse(String(m.args?.payload||'{}'))}catch{}value=!!sendFc({type:String(m.args?.type||''),...data})}else throw new Error('Unknown Python bridge method: '+m.method);worker.postMessage({type:'rpc-result',id:m.id,ok:true,value})}catch(e){worker.postMessage({type:'rpc-result',id:m.id,ok:false,error:String(e?.message||e)})}}
...
function initPython(){
 loadPythonProject();const fallback=$('#pythonEditor');if(fallback)fallback.value=pythonFiles[pythonActiveFile]||PY_DEFAULT;renderPythonFiles();pythonSetRunUi(false);
 ...
 $('#imuCalibrateBtn').onclick=calibrateImuTeaching;$('#imuRecordBtn').onclick=()=>{toggleImuRecording();if(!imuRecording&&$('#imuCsvBtn'))$('#imuCsvBtn').disabled=!imuCsvRows.length};$('#imuCsvBtn').onclick=downloadImuCsv;setInterval(updateImuToolAge,250);window.addEventListener('resize',drawImuTools);
...
async function boot(){
 ...
 window.dispatchEvent(new CustomEvent('zebjus-app-ready',{detail:{three:threeOK,version:'18.3.40',assets}}));
}
