...
setBootStatus('V18.3.44 local-kit module loaded • starting engineering runtime…');
...
function wiringStepProgress(id){
 ...
 const groups={powerWire:requiredWires.filter(([a,b])=>a.startsWith('PDB.E')&&b.includes('PWR')),escFc:requiredWires.filter(([a,b])=>a.startsWith('ESC')&&(/SIG|5V|GND/.test(a))&&b.startsWith('FC.ESC')),xt60:requiredWires.filter(([a])=>a.startsWith('BAT.'))};
...
function previewWiringAction(s){
 const groups={motorWire:requiredWires.filter(([a])=>/ESC\d\.[UVW]/.test(a)),powerWire:requiredWires.filter(([a,b])=>a.startsWith('PDB.E')&&b.includes('PWR')),escFc:requiredWires.filter(([a,b])=>a.startsWith('ESC')&&(/SIG|5V|GND/.test(a))&&b.startsWith('FC.ESC')),xt60:requiredWires.filter(([a])=>a.startsWith('BAT.'))};
...
function simLoop(now){
 ...
 if(s.running&&!remoteFollower){
   ...
   if(keyRoll||keyPitch)setStickVisual('right',cmdR,-cmdP);else if(!s.cmdRoll&&!s.cmdPitch)setStickVisual('right',0,0);
   if(keyYaw)setStickVisual('left',cmdY,(1500-s.throttle)/500);
...
function registerOffline(){
 ...
 navigator.serviceWorker.register('./service-worker.js?v=18.3.44',{updateViaCache:'none'}).then(reg=>reg.update().catch(()=>{})).catch(()=>{});
}
...
const PY_KEYBOARD_RC_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nthrottle = 1000\nprint("Keyboard RC: W/X throttle, arrows roll/pitch, G/K yaw, A arm, D disarm, M toggle ANGLE/RATE. Press Stop to end.")\narmed = False\nmode = "angle"\nlast_toggle = False\nwhile not drone.stop_requested():\n    keys = set(await drone.keys())\n    if "w" in keys: throttle = min(1800, throttle + 8)\n    if "x" in keys: throttle = max(1000, throttle - 12)\n    roll = 1500 + (300 if "ArrowRight" in keys else -300 if "ArrowLeft" in keys else 0)\n    pitch = 1500 + (300 if "ArrowUp" in keys else -300 if "ArrowDown" in keys else 0)\n    yaw = 1500 + (250 if "k" in keys else -250 if "g" in keys else 0)\n    if "a" in keys and throttle <= 1050: armed = True\n    if "d" in keys: armed = False; throttle = 1000\n    toggle = "m" in keys\n    if toggle and not last_toggle: mode = "rate" if mode == "angle" else "angle"\n    last_toggle = toggle\n    await drone.rc(roll=roll, pitch=pitch, throttle=throttle, yaw=yaw, arm=armed, mode=mode)\n    await asyncio.sleep(0.04)\n`;
...
const PY_CAMERA_OPENCV_EXAMPLE=`from zebjus import Drone\nimport asyncio, cv2, numpy as np\n\ndrone = Drone()\nprint("Press Start camera in the Camera / MediaPipe panel first.")\nfor _ in range(15):\n    jpeg = await drone.camera_frame()\n    frame = cv2.imdecode(np.frombuffer(jpeg, dtype=np.uint8), cv2.IMREAD_COLOR)\n    if frame is None: raise RuntimeError("Camera frame could not be decoded")\n    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)\n    edges = cv2.Canny(gray, 80, 160)\n    drone.show_image(cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR))\n    await asyncio.sleep(0.2)\nprint("OpenCV edge project complete")\n`;
const PY_HAND_LANDMARKS_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("Press Start camera and Start hands in Camera / MediaPipe first.")\nfor _ in range(30):\n    data = await drone.hands()\n    for handedness, hand in zip(data["handedness"], data["landmarks"]):\n        tip = hand[8]  # index fingertip, normalized image coordinates\n        print(f"{handedness}: fingertip x={tip['x']:.2f}, y={tip['y']:.2f}")\n    await asyncio.sleep(0.15)\n`;
const PY_CV_TELEMETRY_EXAMPLE=`from zebjus import Drone\nimport asyncio, cv2, numpy as np\n\ndrone = Drone()\nprint("Start camera and connect the selected real kit. Visualization only.")\nfor _ in range(20):\n    jpeg = await drone.camera_frame()\n    frame = cv2.imdecode(np.frombuffer(jpeg, dtype=np.uint8), cv2.IMREAD_COLOR)\n    attitude = await drone.attitude()\n    text = f"R {attitude['roll']:+.1f}  P {attitude['pitch']:+.1f}  Y {attitude['yaw']:+.1f}"\n    cv2.putText(frame, text, (10, 28), cv2.FONT_HERSHEY_SIMPLEX, .55, (55, 235, 150), 2)\n    drone.show_image(frame)\n    await asyncio.sleep(0.2)\n`;
const PY_EXAMPLES=[
 ...
 {id:'keyboard-rc',title:'Keyboard Transmitter',desc:'Keyboard RC → simulator or real FlightCore',code:PY_KEYBOARD_RC_EXAMPLE,target:'sim'},
...
function exportPythonProject(){pythonFiles[pythonActiveFile]=getPythonCode();const payload={schema:1,product:'ZEBJUS Python Lab',version:'18.3.44',exportedAt:new Date().toISOString(),active:pythonActiveFile,files:pythonFiles};downloadBlob('ZEBJUS_Python_Project.json',JSON.stringify(payload,null,2),'application/json')}
...
function pythonTerminalWrite(text,kind='out'){const term=$('#pythonTerminal');if(!term)return;const value=String(text||'');if(!value)return;const span=document.createElement('span');span.className=kind==='error'?'python-terminal-error':kind==='warn'?'python-terminal-warn':'python-terminal-out';span.textContent=value;term.appendChild(span);if(term.textContent.length>64000){const keep=term.textContent.slice(-56000);term.textContent='… older terminal output trimmed …\n'+keep}term.scrollTop=term.scrollHeight}
...
function pythonPhysicalSensorNeeded(code){return /\b(?:i2c_scan|imu\s*\(|gyro\s*\(|accel\s*\()/.test(String(code||''))}
function pythonKitCommandNeeded(code){return /\b(?:pid_get|set_rate_pid|set_angle_pid|restore_pid_defaults|get_calibration|set_accel_offsets|level_calibrate|restore_calibration_defaults|receiver\s*\(|ppm\s*\(|rc\s*\(|motor_test|motor_order_test|motor_stop|esc_calibrate|bench_status|calibrate_gyro|attitude\s*\()/.test(String(code||''))}
function pythonHardwarePreflight(code){
 const target=$('#pythonTarget')?.value||'sim',physical=pythonPhysicalSensorNeeded(code),kitCommand=pythonKitCommandNeeded(code);if(target==='sim'&&physical){const term=$('#pythonTerminal');if(term){term.textContent='';pythonTerminalWrite('[TARGET] Physical I²C/IMU reads require “Real ZEBJUS kit”.\n','error')}$('#pyStatus').textContent='Real hardware target required';return false}
...
let pythonCameraStream=null,pythonHandLandmarker=null,pythonHandTimer=0,pythonCameraCanvas=null,pythonRealRcActive=false,pythonLastHardwareCommand=Promise.resolve();let pythonHandData={landmarks:[],handedness:[],timestampMs:0};
function pythonCameraNote(message){const n=$('#pythonCameraStatus');if(n)n.textContent=message}
async function startPythonCamera(){if(pythonCameraStream)return;const video=$('#pythonCameraVideo');if(!video||!navigator.mediaDevices?.getUserMedia)throw new Error('Camera requires HTTPS or localhost with browser permission.');pythonCameraStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:{ideal:640},height:{ideal:480}},audio:false});video.srcObject=pythonCameraStream;await video.play();pythonCameraNote('Camera ready • use drone.camera_frame() in Python.');}
...
function pythonVisualImage(m){const mime=m.mimeType==='image/jpeg'?'image/jpeg':'image/png',b64=String(m.base64||'');if(!b64||b64.length>3000000){pythonTerminalWrite('[IMAGE] Output was empty or exceeded 2 MB. Reduce plot size.\n','warn');return}const src=`data:${mime};base64,${b64}`,img=$('#pythonPlotImage'),link=$('#pythonPlotDownload');if(img){img.src=src;img.hidden=false;img.closest('details').open=true}if(link){link.href=src;link.download=`zebjus-python-output.${mime==='image/jpeg'?'jpg':'png'}`;link.hidden=false}}
...
async function boot(){
 ...
 window.dispatchEvent(new CustomEvent('zebjus-app-ready',{detail:{three:threeOK,version:'18.3.44',assets}}));
}
