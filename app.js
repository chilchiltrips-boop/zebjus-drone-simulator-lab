...
setBootStatus('V17.5.1 local module loaded • starting adaptive multi-screen final build…');
...
function simLoop(now){
 ...
 const base=s.running?clamp((s.throttle-1000)/10,0,100):0,rc=s.running?clamp(uR*.045,-30,30):0,pc=s.running?clamp(uP*.045,-30,30):0,yc=s.running?clamp(uY*.028,-20,20):0;
 const cmdMix=s.running?[base+rc-pc+yc,base-rc-pc-yc,base-rc+pc+yc,base+rc+pc-yc].map(v=>clamp(v,0,100)):[0,0,0,0];
 const lag=Math.max(.035,s.motorLag),aLag=s.running?1-Math.exp(-dt/lag):1;
 s.motorActual=s.motorActual.map((v,i)=>s.running?v+(cmdMix[i]-v)*aLag:0);const mix=s.motorActual;s.lastMix=mix;
...
function projectPayload(){return{version:'17.5.1',savedAt:new Date().toISOString(),guided:state.guided,step:state.step,parts:state.parts.filter(p=>!p.internal&&p.type!=='batteryStrap').map(p=>({type:p.type,slotId:p.slotId})),actions:[...state.doneActions],connections:state.connections,pid:state.pid,wireLayout,wireNodeTransforms,optionalWireNodes,sim:{batteryV:state.sim.batteryV,payloadG:state.sim.payloadG,cgX:state.sim.cgX,cgY:state.sim.cgY,wind:state.sim.wind,motorLag:state.sim.motorLag}}}
function exportProjectJson(){downloadBlob('ZEBJUS_F450_Project_V17_4.json',JSON.stringify(projectPayload(),null,2),'application/json')}
function importProjectJson(file){const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(r.result);localStorage.setItem('zebjusF450V1751',JSON.stringify(d));notify('Project imported • reloading.','good');setTimeout(()=>location.reload(),450)}catch(e){notify('Invalid project JSON.','bad')}};r.readAsText(file)}
...
function boot(){
 ...
 window.dispatchEvent(new CustomEvent('zebjus-app-ready',{detail:{three:threeOK,version:'17.5.1'}}));
}
boot();
