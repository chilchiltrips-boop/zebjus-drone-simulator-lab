...
setBootStatus('V18.3.41 local-kit module loaded • starting engineering runtime…');
...
function applyQuickPid(){
 ...
 markPidDirty();resetSimControllersOnly();renderPid();updatePidCoach();notify(`${state.sim.flightMode==='rate'?'RATE-mode PID':'ANGLE outer + inner Rate PID'} applied.`)
}
...
function stopSimRuntime(updateButton=true){
 state.sim.running=false;heldKeys.clear();
...
function projectPayload(){return{version:'18.3.41',savedAt:new Date().toISOString(),guided:state.guided,step:state.step,parts:state.parts.filter(p=>!p.internal&&p.type!=='batteryStrap').map(p=>({type:p.type,slotId:p.slotId})),actions:[...state.doneActions],connections:state.connections,pid:state.pid,wireLayout,wireNodeTransforms,optionalWireNodes,sim:{batteryV:state.sim.batteryV,payloadG:state.sim.payloadG,cgX:state.sim.cgX,cgY:state.sim.cgY,wind:state.sim.wind,motorLag:state.sim.motorLag}}}
...
function registerOffline(){
 ...
 navigator.serviceWorker.register('./service-worker.js?v=18.3.41',{updateViaCache:'none'}).then(reg=>reg.update().catch(()=>{})).catch(()=>{});
}
...
const PY_EXAMPLES=[
 ...
 {id:'angle',title:'Angle / Attitude Read',desc:'Roll, pitch and yaw attitude',code:PY_ANGLE_EXAMPLE,target:'sim'},
 ...
 {id:'level-check',title:'Level Checker',desc:'Live roll/pitch + total tilt check',code:PY_LEVEL_CHECK_EXAMPLE,target:'sim'},
...
function exportPythonProject(){pythonFiles[pythonActiveFile]=getPythonCode();const payload={schema:1,product:'ZEBJUS Python Lab',version:'18.3.41',exportedAt:new Date().toISOString(),active:pythonActiveFile,files:pythonFiles};downloadBlob('ZEBJUS_Python_Project.json',JSON.stringify(payload,null,2),'application/json')}
...
function calibrateImuTeaching(){if(imuToolSamples.length<20){notify('Collect at least 20 IMU samples while the sensor is level and still.','bad');return}const a=imuToolSamples.slice(-60),avg=k=>a.reduce((n,v)=>n+v[k],0)/a.length;imuCal={ax:avg('ax'),ay:avg('ay'),az:avg('az')-1,gx:avg('gx'),gy:avg('gy'),gz:avg('gz')};try{localStorage.setItem('zebjus-imu-teaching-cal',JSON.stringify(imuCal))}catch{};$('#imuToolNote').textContent=`Browser teaching offset saved • gyro ${imuCal.gx.toFixed(2)}, ${imuCal.gy.toFixed(2)}, ${imuCal.gz.toFixed(2)} dps. Firmware flight calibration is still pending.`;drawImuTools();notify('Browser IMU level offset saved.','good')}
...
function pythonKitCommandNeeded(code){return /\b(?:pid_get|set_rate_pid|set_angle_pid|restore_pid_defaults|receiver\s*\(|ppm\s*\(|rc\s*\(|motor_test|motor_order_test|motor_stop|esc_calibrate|bench_status|calibrate_gyro|attitude\s*\()/.test(String(code||''))}
...
function pythonFriendlyError(error,stderr=''){
 ...
 if(/PROPS_REMOVED/i.test(all))return 'Bench motor/ESC command blocked. REMOVE PROPELLERS, keep the frame secured, then run the example with confirm="PROPS_REMOVED".';
 if(/Take Control/i.test(all))return 'The kit is connected in View Only mode. Press Take Control for this Device ID, then Run again.';
...
async function runPythonLab(){
 ...
 pythonFiles[pythonActiveFile]=getPythonCode();savePythonProject();const code=getPythonCode();if(!pythonHardwarePreflight(code))return;clearPythonMarkers();pythonStderrBuffer='';pythonStopRequested=false;pythonHasRun=true;pythonSetRunUi(true);const target=$('#pythonTarget')?.value||'sim';if(target==='sim'&&syncPythonCodeToSimulator(code))pythonTerminalWrite('[SIM SYNC] PID constants from this file were applied to Tripod Simulator.\n','warn');const worker=new Worker(`./python-worker.js?v=18.3.41`);pythonWorker=worker;const runId=++pythonWorkerRunId;$('#pyStatus').textContent=`Starting ${pythonActiveFile} in isolated Worker…`;pythonTerminalWrite(`\n▶ ${pythonActiveFile} • ${target==='real'?'REAL KIT':'SIMULATOR'}\n`);
...
async function boot(){
 ...
 window.dispatchEvent(new CustomEvent('zebjus-app-ready',{detail:{three:threeOK,version:'18.3.41',assets}}));
}
