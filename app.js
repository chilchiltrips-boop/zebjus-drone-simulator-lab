...
setBootStatus('V18 school-cloud module loaded • starting classroom engineering runtime…');
...
function projectPayload(){return{version:'18.0.0',savedAt:new Date().toISOString(),guided:state.guided,step:state.step,parts:state.parts.filter(p=>!p.internal&&p.type!=='batteryStrap').map(p=>({type:p.type,slotId:p.slotId})),actions:[...state.doneActions],connections:state.connections,pid:state.pid,wireLayout,wireNodeTransforms,optionalWireNodes,sim:{batteryV:state.sim.batteryV,payloadG:state.sim.payloadG,cgX:state.sim.cgX,cgY:state.sim.cgY,wind:state.sim.wind,motorLag:state.sim.motorLag}}}
...
function sendFc(o){if(window.zebjusSchool?.isViewOnly?.()){fcLog('VIEW ONLY • command blocked');return false}if(window.zebjusSchool?.isCloudActive?.()){if(window.zebjusSchool.sendDeviceCommand?.(o)){fcLog('CLOUD TX '+JSON.stringify(o));return true}fcLog('Cloud module is not locked / controllable');return false}if(state.fc.socket?.readyState===1){state.fc.socket.send(JSON.stringify(o));fcLog('TX '+JSON.stringify(o));return true}fcLog('Not connected');return false}
...
# For a locked real module, commands route by Device ID through School Cloud:
...
// V18 public bridge used by School Cloud / classroom view-only runtime.
window.zebjusLabAPI={
 version:'18.0.0',
...
function boot(){
 ...
 window.dispatchEvent(new CustomEvent('zebjus-app-ready',{detail:{three:threeOK,version:'18.0.0'}}));
}
boot();
