...
setBootStatus('V18.3.46 local-kit module loaded • starting engineering runtime…');
...
// The four physical ESC headers stay in place; software routes each logical motor to one.
function fcHeaderForMotor(i){
 const connector=window.zebjusExpansion?.motors?.[i-1]?.connector;
 const slot=/^D[0-3]$/.test(connector)?Number(connector.slice(1)):[1,2,3,0][i-1];
 return ({0:4,1:1,2:2,3:3})[slot];
}
function referenceWires(){return requiredWires.map(([from,to])=>{
 const m=/^ESC([1-4])\.(?:SIG|5V|GND)$/.exec(from);
 return m?[from,to.replace(/^FC\.ESC[1-4]/,`FC.ESC${fcHeaderForMotor(Number(m[1]))}`)]:[from,to];
})}
window.addEventListener('zebjus-expansion-change',()=>{try{render2D();renderAssemblyUI()}catch(e){console.warn('Expansion wiring update:',e)}});
...
const slots={
 ...
 receiver:[{id:'RX',p:[1.36,1.33,-.48],r:0}],gps:[{id:'GPS',p:[0,2.18,.82],r:0}],servo:[{id:'SV1',p:[1.58,1.25,-1.10],r:0},{id:'SV2',p:[-1.58,1.25,-1.10],r:0}],matrix:[{id:'MATRIX',p:[-1.52,1.34,-.52],r:rad(90)}],sensor:[{id:'SEN1',p:[-1.30,1.34,.50],r:0},{id:'SEN2',p:[1.30,1.34,.52],r:0}],led:[{id:'LED1',p:[.78,1.34,1.03],r:0},{id:'LED2',p:[-.78,1.34,1.03],r:0}],
...
/* Local/offline GLB runtime. Every packaged component model is preloaded before
...
function cloneAsset(path){
 const src=assetTemplates.get(path);if(!src)return null;const root=src.clone(true);root.traverse(o=>{if(o.isMesh&&o.material){o.material=Array.isArray(o.material)?o.material.map(m=>m.clone()):o.material.clone()}});return root
}


/* ==================== V13 SOUND / POWER FX ==================== */
let audioCtx=null,powerSequenceToken=0,activeTones=new Set();
...
function tone(freq=440,dur=.08,type='sine',gain=.025,delay=0){
 ...
 const level=Math.max(.0004,gain*soundVolume);
 ...
 const rec={o,g,f};activeTones.add(rec);
 o.onended=()=>{activeTones.delete(rec);try{o.disconnect();f.disconnect();g.disconnect()}catch{}};
 o.start(t);o.stop(t+dur+.035)
}
function stopTransientTones(){
 const ac=audioCtx,t=ac?.currentTime||0;
 activeTones.forEach(a=>{
...
function updateMotorAudio(kind,level,imbalance=0){
 ...
 if(!soundEnabled||soundVolume<=0||l<=.001){
...
function connectBatteryPower(fromGuided=false){
 ...
 state.doneActions.add('xt60');silenceMotorAudio('assembly');animateBatteryPlug(true);playFX('connector');powerConnectThunk();setTimeout(()=>{if(state.powered)xt60Spark()},340);rebuild3DWires();rebuildSolder();render2D();renderAssemblyUI();validate2D();
 notify('XT60 inserted • power flowing to ESCs and FC…','good');runPowerUpSequence();return true
}
function disconnectBatteryPower(){
 powerSequenceToken++;stopTransientTones();destroyMotorAudio('assembly');
...
let adaptiveTimer=0,adaptiveRO=null,adaptiveMode='desktop';
function recommendedPerformanceMode(){
 const w=Math.max(window.innerWidth||0,window.screen?.width||0),coarse=window.matchMedia?.('(pointer:coarse)')?.matches,mem=navigator.deviceMemory||8;
 if(w>=2200)return'high';
 if(w<=700&&(coarse||mem<=4))return'low';
 return'balanced'
}
function adaptiveDeviceMode(){
 const w=window.innerWidth||document.documentElement.clientWidth||1200,h=window.innerHeight||document.documentElement.clientHeight||800;
 if(w>=2200)return'tv';
 if(w>=1200)return'desktop';
 if(w>=700)return'tablet';
 return'mobile'
}
function refreshAdaptiveCanvases(){
 try{resize3D?.()}catch{}
 try{resizeSim?.()}catch{}
 if($('#tab-wiring')?.classList.contains('active'))try{render2D?.()}catch{}
}
function updateAdaptiveLayout(){
 const w=window.innerWidth||document.documentElement.clientWidth||1200,h=window.innerHeight||document.documentElement.clientHeight||800,mode=adaptiveDeviceMode(),coarse=window.matchMedia?.('(pointer:coarse)')?.matches;
 adaptiveMode=mode;
...
function initAdaptiveLayout(){
 ...
 const mq=window.matchMedia?.('(pointer:coarse)');mq?.addEventListener?.('change',run);
 if('ResizeObserver'in window){
   adaptiveRO=new ResizeObserver(()=>{clearTimeout(adaptiveTimer);adaptiveTimer=setTimeout(refreshAdaptiveCanvases,80)});
   ['#threeContainer','#simThree','.svg-wrap'].forEach(s=>{const e=$(s);if(e)adaptiveRO.observe(e)})
 }
}
function initTabs(){
 $$('.tab').forEach(b=>b.onclick=()=>{
   const leavingSim=$('#tab-sim')?.classList.contains('active')&&b.dataset.tab!=='sim';
   const leavingWire=$('#tab-wiring')?.classList.contains('active')&&b.dataset.tab!=='wiring';
   if(leavingSim)stopSimRuntime(true);
   if(leavingWire&&wireMotorRun){wireMotorRun=false;destroyMotorAudio('wire');render2D()}
   heldKeys.clear();
   $$('.tab').forEach(x=>x.classList.remove('active'));b.classList.add('active');
   $$('.tab-panel').forEach(x=>x.classList.remove('active'));$('#tab-'+b.dataset.tab)?.classList.add('active');
   if(b.dataset.tab==='assembly')setTimeout(resize3D,40);
   if(b.dataset.tab==='sim'){ensureSim();setTimeout(resizeSim,80)}
 })
}
...
const wiringActionIds=new Set(['motorWire','powerWire','escFc','xt60']);
function pairPresent(a,b){return state.connections.some(c=>(c.from===a&&c.to===b)||(c.from===b&&c.to===a))}
function partQty(type){return state.parts.filter(p=>p.type===type).length}
function wiringPrerequisitesReady(id){
 if(id==='motorWire')return partQty('esc')>=4&&partQty('motor')>=4;
 if(id==='powerWire')return partQty('bottomPlate')>=1&&partQty('esc')>=4;
 if(id==='escFc')return partQty('fc')>=1&&partQty('esc')>=4;
 if(id==='xt60')return partQty('bottomPlate')>=1&&partQty('battery')>=1;
 return true
}
function wiringStepProgress(id){
 if(id==='motorWire'){let n=0;for(let i=1;i<=4;i++){for(const p of ['U','V','W'])if(state.connections.some(c=>{const a=c.from.split('.'),b=c.to.split('.');return(a[0]===`ESC${i}`&&a[1]===p&&b[0]===`M${i}`&&['U','V','W'].includes(b[1]))||(b[0]===`ESC${i}`&&b[1]===p&&a[0]===`M${i}`&&['U','V','W'].includes(a[1]))}))n++}return n}
 const groups={powerWire:referenceWires().filter(([a,b])=>a.startsWith('PDB.E')&&b.includes('PWR')),escFc:referenceWires().filter(([a,b])=>a.startsWith('ESC')&&(/SIG|5V|GND/.test(a))&&b.startsWith('FC.ESC')),xt60:referenceWires().filter(([a])=>a.startsWith('BAT.'))};
 return (groups[id]||[]).filter(([a,b])=>pairPresent(a,b)).length
}
function wiringActionReady(id){
 if(!wiringPrerequisitesReady(id))return false;
 if(id==='motorWire')return [1,2,3,4].every(i=>phaseMap(i));
 if(id==='powerWire')return wiringStepProgress(id)>=8;
 if(id==='escFc')return wiringStepProgress(id)>=12;
 if(id==='xt60')return wiringStepProgress(id)>=2;
 return false
}
function syncWiringActionsToAssembly(){
 let changed=false;
 for(const id of wiringActionIds){const ready=wiringActionReady(id),has=state.doneActions.has(id);if(ready&&!has){state.doneActions.add(id);changed=true}else if(!ready&&has){state.doneActions.delete(id);changed=true}}
 const xt=wiringActionReady('xt60');if(xt!==state.powered)setPowerVisual(xt,true);
 return changed
}
...
function procProp(id){
 const g=new THREE.Group(),handed=(id==='M2'||id==='M4')?-1:1,isCW=handed>0;
 const bladeMat=mat(0xdbe5eb,.16,.30),edgeMat=mat(isCW?0x55d7ff:0xffb15b,.12,.34),hubMat=mat(0xaebbc4,.72,.20),dark=mat(0x3a4650,.42,.28);
 function bladeGeometry(){
   const sh=new THREE.Shape();sh.moveTo(.10,-.105);sh.bezierCurveTo(.36,-.20,.92,-.255,1.36,-.135);sh.bezierCurveTo(1.55,-.085,1.61,-.018,1.56,.055);sh.bezierCurveTo(1.37,.19,.88,.255,.42,.185);sh.bezierCurveTo(.26,.158,.15,.132,.10,.105);sh.closePath();
   const geo=new THREE.ExtrudeGeometry(sh,{depth:.050,bevelEnabled:true,bevelThickness:.010,bevelSize:.015,bevelSegments:2});geo.rotateX(Math.PI/2);return geo
 }
 const geo=bladeGeometry();
 [0,Math.PI].forEach(a=>{const holder=new THREE.Group();holder.rotation.y=a;g.add(holder);const b=new THREE.Mesh(geo,bladeMat);b.position.y=.11;b.rotation.x=handed*rad(7.5);b.castShadow=true;b.receiveShadow=true;holder.add(b);const tip=B(.28,.052,.17,edgeMat,[1.38,.14,0],holder,[handed*rad(7.5),0,0]);tip.castShadow=true});
 CY(.21,.12,hubMat,[0,.07,0],g);CY(.105,.28,hubMat,[0,.24,0],g);CY(.16,.105,dark,[0,.42,0],g);
 const mark=new THREE.Mesh(new THREE.TorusGeometry(.25,.022,8,32),new THREE.MeshStandardMaterial({color:isCW?0x55d7ff:0xffb15b,metalness:.08,roughness:.36}));mark.rotation.x=Math.PI/2;mark.position.y=.145;g.add(mark);g.userData.propDirection=isCW?'CW':'CCW';
 return g
}
...
function decoratePart(g,type,id){
 if(type==='bottomPlate'&&!g.getObjectByName('FRAME_RISER_1')){
   const postMat=mat(0x303a42,.48,.28);[[-.95,-.72],[.95,-.72],[-.95,.72],[.95,.72]].forEach(([x,z],i)=>{const post=CY(.075,.30,postMat,[x,.29,z],g);post.name=`FRAME_RISER_${i+1}`;const cap=CY(.105,.035,mat(0x11181e,.35,.35),[x,.445,z],g);cap.name=`FRAME_RISER_CAP_${i+1}`});
 }
...
function init3D(){
 ...
 scene=new THREE.Scene();scene.background=new THREE.Color(0x112431);
 ...
 renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.18;e.appendChild(renderer.domElement);
 ...
 scene.add(new THREE.HemisphereLight(0xf2fbff,0x71818b,1.28));
 const key=new THREE.DirectionalLight(0xffffff,2.35);key.position.set(6,10,8);key.castShadow=true;scene.add(key);
 const fill=new THREE.DirectionalLight(0xc9e6ff,1.15);fill.position.set(-7,6,4);scene.add(fill);
 const rim=new THREE.DirectionalLight(0x9beeff,.82);rim.position.set(-4,5,-8);scene.add(rim);
 const warm=new THREE.PointLight(0xffe2bf,.88,18);warm.position.set(4.8,5.8,1.8);scene.add(warm);
 const frontFill=new THREE.PointLight(0xc4e8ff,.72,16);frontFill.position.set(-4.5,3.8,5.5);scene.add(frontFill);
...
function snapRadius(type){const base=({bottomPlate:120,topPlate:110,battery:120,armRed:115,armWhite:115,guard:125,motor:105,esc:115,fcTape:95,fc:95,prop:115,receiver:100,gps:105,servo:100,matrix:100,sensor:100,led:95}[type]??105),r=renderer?.domElement?.getBoundingClientRect?.();return base*clamp(Math.min(r?.width||800,r?.height||800)/800,.85,1.35)}
function pointerCanvasPoint(e){const r=renderer.domElement.getBoundingClientRect();return{x:e.clientX-r.left,y:e.clientY-r.top,w:r.width,h:r.height}}
function projectSlotToScreen(slot){const r=renderer.domElement.getBoundingClientRect(),v=new THREE.Vector3(slot.p[0],slot.p[1],slot.p[2]).project(camera);return{x:(v.x*.5+.5)*r.width,y:(-.5*v.y+.5)*r.height,z:v.z,visible:v.z>=-1&&v.z<=1}}
function nearestFreeSlotScreen(type,e){
 ...
 const p=pointerCanvasPoint(e);return free.map(s=>{const q=projectSlotToScreen(s);return{s,d:Math.hypot(q.x-p.x,q.y-p.y),screen:q}}).filter(x=>x.screen.visible).sort((a,b)=>a.d-b.d)[0]||null
}
...
function move(e){
 ...
 const n=nearestFreeSlotScreen(state.selectedType,e);if(!n){snapPreview.visible=false;return}
...
function findSlot(type,e){const n=nearestFreeSlotScreen(type,e);return n?.s||null}
function placePointer(e){
 ...
 const near=nearestFreeSlotScreen(type,e),s=near?.s;if(!s){notify('No visible free snap point in this view.','bad');return}
 if(near.d>snapRadius(type)){notify(`Move closer to the highlighted ${s.id} snap target.`,'bad');return}
...
function resize3D(){if(!renderer||!camera)return;const e=$('#threeContainer');if(!e)return;const r=e.getBoundingClientRect(),w=Math.max(1,Math.round(e.clientWidth||r.width||900)),h=Math.max(1,Math.round(e.clientHeight||r.height||600));camera.aspect=w/h;camera.updateProjectionMatrix();renderer.setSize(w,h,false)}
function loop3D(t){
 ...
 wiresRoot.visible=state.connections.length>0||state.wireMap||state.xray;
...
function rebuild3DWires(){
 ...
 state.connections.forEach(c=>{
   const phase=/\.(?:U|V|W)$/.test(c.from)||/\.(?:U|V|W)$/.test(c.to),pts=routePoints(c),th=/BAT|PWR/.test(c.from+c.to) ? .045 : (phase ? .024 : .018);
   const w=tube(pts,wColor(c.from),th,wiresRoot);tag(w,{wire:`${c.from} → ${c.to}`,wireId:c.id});
   if(phase){const metal=new THREE.MeshStandardMaterial({color:0xd7a63c,metalness:.78,roughness:.22});[pts[0],pts[pts.length-1]].forEach(pt=>{const plug=new THREE.Mesh(new THREE.SphereGeometry(.050,12,8),metal);plug.position.copy(pt);plug.castShadow=true;wiresRoot.add(plug)})}
...
/* actions */
function addGroup(g){
 const groups={motor:referenceWires().filter(([a])=>/ESC\d\.[UVW]/.test(a)),power:referenceWires().filter(([a,b])=>a.startsWith('PDB.E')&&b.includes('PWR')),escfc:referenceWires().filter(([a,b])=>a.startsWith('ESC')&&(/SIG|5V|GND/.test(a))&&b.startsWith('FC.ESC')),xt60:referenceWires().filter(([a])=>a.startsWith('BAT.')),rx:referenceWires().filter(([a])=>a.startsWith('RX.'))};const arr=groups[g]||[];arr.forEach(([from,to],i)=>{if(!state.connections.some(c=>c.from===from&&c.to===to))state.connections.push({from,to,new:true});if(g!=='escfc'||from.endsWith('.SIG'))setTimeout(()=>animatePlug(from,to),i*80)});setTimeout(()=>state.connections.forEach(c=>c.new=false),1400);render2D();rebuild3DWires();rebuildSolder()
}
function previewWiringAction(s){
 const groups={motorWire:referenceWires().filter(([a])=>/ESC\d\.[UVW]/.test(a)),powerWire:referenceWires().filter(([a,b])=>a.startsWith('PDB.E')&&b.includes('PWR')),escFc:referenceWires().filter(([a,b])=>a.startsWith('ESC')&&(/SIG|5V|GND/.test(a))&&b.startsWith('FC.ESC')),xt60:referenceWires().filter(([a])=>a.startsWith('BAT.'))};
 const arr=groups[s.id]||[];if(!wiringPrerequisitesReady(s.id)){notify('Install the required physical components first, then use the 2D Wiring page.','bad');return}
 arr.forEach(([from,to],i)=>setTimeout(()=>animatePlug(from,to),i*85));
 notify('Preview only • no assembly progress was changed. Draw these connections in 2D Wiring.','good')
}
function performAction(s){
 if(wiringActionIds.has(s.id)){previewWiringAction(s);return}
 if(s.id==='inspect'&&!steps.slice(0,steps.length-1).every((_,i)=>stepDone(i))){notify('Complete previous steps first.','bad');return}
 if(s.id==='inspect'&&typeof electricalIssues==='function'&&electricalIssues().some(x=>x.level==='bad')){notify('Final inspection blocked • fix critical electrical validation errors first.','bad');return}
 historyPush();state.doneActions.add(s.id);rebuildSolder();renderAssemblyUI();notify(`${s.title} completed.`)
}

/* ==================== V8 INTERACTIVE 2D WIRING BENCH/* ==================== V8 INTERACTIVE 2D WIRING BENCH ==================== */
...
function nodeTransformAttr(id,w,h){
 const p=nodePos(id),t=wireTransform(id),cx=w/2,cy=h/2;
...
function nodeCanvasPoint(id,w,h,x,y){
 const p=nodePos(id),t=wireTransform(id),cx=w/2,cy=h/2,r=rad(t.rot||0),dx=x-cx,dy=y-cy;
...
function flipSelectedNode(){
 if(!selectedWireNodeId){notify('Select an object first.','bad');return}
 if(selectedWireNodeId==='FC'){notify('FC pin map is fixed; FC is not mirrored.','bad');return}
 wireRemember();const t=wireTransform(selectedWireNodeId);
 wireNodeTransforms[selectedWireNodeId]={...t,flipX:!t.flipX};
 persistWireLayout();render2D();notify('Connector side flipped • text remains readable.','good')
}
...
function drawNode(svg,id,title,w,h,portsLeft=[],portsRight=[],sub='DRAG TO MOVE',extraClass=''){
 const t=wireTransform(id),flip=!!t.flipX,leftPorts=flip?portsRight:portsLeft,rightPorts=flip?portsLeft:portsRight;
 ...
 g.appendChild(E('rect',{x:0,y:0,width:w,height:h,rx:12}));
 g.appendChild(E('rect',{x:0,y:0,width:w,height:28,rx:12,class:'drag-handle'}));
 g.appendChild(E('text',{x:11,y:18,class:'node-title'},title+nodeLabelSuffix(id)));
 g.appendChild(E('text',{x:11,y:h-8,class:'node-sub'},sub));
 if(flip)g.appendChild(E('text',{x:w-12,y:18,class:'flip-badge-2d','text-anchor':'end'},'FLIPPED'));
 leftPorts.forEach((q,i)=>{const yy=42+i*20;g.appendChild(E('text',{x:14,y:yy+3,class:'port-label-v8'},q[0]));const c=E('circle',{cx:0,cy:yy,r:6.5,class:`port-v8 ${q[1]}${selPort===id+'.'+q[0]?' selected':''}`});c.dataset.port=id+'.'+q[0];c.onclick=e=>{e.stopPropagation();choosePort(c.dataset.port)};g.appendChild(c);svgPorts[id+'.'+q[0]]=nodeCanvasPoint(id,w,h,0,yy)});
 rightPorts.forEach((q,i)=>{const yy=42+i*20;g.appendChild(E('text',{x:w-14,y:yy+3,class:'port-label-v8','text-anchor':'end'},q[0]));const c=E('circle',{cx:w,cy:yy,r:6.5,class:`port-v8 ${q[1]}${selPort===id+'.'+q[0]?' selected':''}`});c.dataset.port=id+'.'+q[0];c.onclick=e=>{e.stopPropagation();choosePort(c.dataset.port)};g.appendChild(c);svgPorts[id+'.'+q[0]]=nodeCanvasPoint(id,w,h,w,yy)});
 g.onpointerdown=e=>beginNodeDrag(id,e);svg.appendChild(g);return g
}
function drawMotorNode(svg,id,title){
 const w=150,h=100,phase=[['U','u'],['V','v'],['W','w']],phaseLeft=(id==='M1'||id==='M4');
 const g=phaseLeft?drawNode(svg,id,title,w,h,phase,[],'PHASE LEADS ← ESC • F flip • R rotate'):
                   drawNode(svg,id,title,w,h,[],phase,'ESC → PHASE LEADS • F flip • R rotate');
 const holder=E('g',{transform:'translate(75 62)'}),rotor=E('g',{id:`motorRotor${id.slice(1)}`,class:'motor-rotor-2d'});
 rotor.appendChild(E('circle',{cx:0,cy:0,r:25,class:'motor-ring-2d'}));
 rotor.appendChild(E('rect',{x:-39,y:-3,width:78,height:6,rx:3,class:'motor-blade-2d'}));
 rotor.appendChild(E('rect',{x:-3,y:-39,width:6,height:78,rx:3,class:'motor-blade-2d'}));holder.appendChild(rotor);g.appendChild(holder);
 const effectiveLeft=phaseLeft!==!!wireTransform(id).flipX,edge=effectiveLeft?0:w;
 [['#f5c542',-8],['#2c92ff',0],['#87949d',8]].forEach(([col,dy])=>g.appendChild(E('path',{d:effectiveLeft?`M52 ${62+dy} Q30 ${62+dy} ${edge} ${62+dy}`:`M98 ${62+dy} Q120 ${62+dy} ${edge} ${62+dy}`,stroke:col,'stroke-width':2.5,fill:'none',class:'motor-pigtail-2d'})));
 g.appendChild(E('text',{x:75,y:95,id:`motorDir${id.slice(1)}`,class:'motor-dir-2d'},'OPEN'));return g
}
function drawFCNode(svg){
 ...
 g.appendChild(E('rect',{x:35,y:172,width:178,height:78,rx:7,class:'fc-block-v14 esc'}));g.appendChild(E('text',{x:124,y:166,'text-anchor':'middle',class:'fc-section-label'},'ESC HEADERS • D1 D2 D3 D0'));
 const ex=[58,100,142,184],ey=[190,214,238];['SOURCE','+5V','GND'].forEach((n,r)=>g.appendChild(E('text',{x:28,y:ey[r]+3,'text-anchor':'end',class:`fc-row-label ${r===0?'source':r===1?'fivev':'ground'}`},n)));ex.forEach((x,i)=>ey.forEach((y,r)=>makePin(`FC.ESC${i+1}-${r===0?'S':r===1?'5V':'G'}`,x,y,r===0?'signal':r===1?'fivev':'ground',r===0?`D${[1,2,3,0][i]}`:'')));
...
function electricalIssues(){
 ...
 const isHigh=k=>/^BAT\.\+$|^PDB\.(BAT\+|E\d\+)$|^ESC\d\.PWR\+$/.test(k),isGround=k=>/^BAT\.-$|^PDB\.(BAT-|E\d-)$|^ESC\d\.PWR-$|GND$|-G$/.test(k),isLow=k=>/^FC\.(ESC|GPIO|RX|I2C)/.test(k);
 conns.forEach(c=>{
 ...
 for(let i=1;i<=4;i++){
   ...
   if(sig){const other=sig.from===`ESC${i}.SIG`?sig.to:sig.from;if(other!==`FC.ESC${fcHeaderForMotor(i)}-S`)issues.push({level:'bad',text:`ESC${i} signal is on ${other}; expected FC.ESC${fcHeaderForMotor(i)}-S.`})}
   const anyCtrl=['SIG','5V','GND'].some(p=>conns.some(c=>c.from===`ESC${i}.${p}`||c.to===`ESC${i}.${p}`));
   if(anyCtrl&&!conns.some(c=>(c.from===`ESC${i}.GND`&&c.to===`FC.ESC${fcHeaderForMotor(i)}-G`)||(c.to===`ESC${i}.GND`&&c.from===`FC.ESC${fcHeaderForMotor(i)}-G`)))issues.push({level:'warn',text:`ESC${i} control ground is missing.`});
...
function updateWireAssemblySync(){
 const box=$('#wireAssemblySync'),badge=$('#wireSyncBadge');if(!box)return;
 const rows=[['motorWire','Motor U / V / W',12],['powerWire','ESC power → PDB',8],['escFc','ESC control → FC',12],['xt60','Battery XT60',2]];
 box.innerHTML=rows.map(([id,label,need])=>{const n=wiringStepProgress(id),done=wiringActionReady(id),pr=wiringPrerequisitesReady(id);return `<div class="sync-row ${done?'done':''}"><span><b>${label}</b><span>${pr?`${n}/${need} correct connection(s)`: 'Install prerequisite parts in Assembly first'}</span></span><em>${done?'ASSEMBLY ✓':pr?'DRAW':'WAIT'}</em></div>`}).join('');
 if(badge){const complete=rows.filter(([id])=>wiringActionReady(id)).length;badge.textContent=`${complete}/4 SYNC`;badge.className='status '+(complete===4?'good':'')}
}
function validate2D(){
 const b=$('#wireValidation');if(!b)return;
 const fixed=referenceWires().filter(([a,z])=>!/\.[UVW]$/.test(a)&&!/\.[UVW]$/.test(z)),missingPairs=fixed.filter(([a,z])=>!conn(a,z)),batteryMissing=missingPairs.filter(([a,z])=>a.startsWith('BAT.')||z.startsWith('BAT.')).length,nonBatteryMissing=missingPairs.length-batteryMissing,phaseReady=[1,2,3,4].filter(i=>phaseMap(i)).length,issues=electricalIssues();
...
function initWiringControls(){
 ...
 $('#wireRunBtn').onclick=()=>{wireMotorRun=!wireMotorRun;if(wireMotorRun)ensureMotorAudio('wire');else destroyMotorAudio('wire');render2D()};
...
window.zebjusGetKeyMap=()=>({...keyMap});
const heldKeys=new Set();let keyCaptureAction=null;
function keyLabel(k){return k===' '?'Space':k}
function simClone(type,slotId){const c=product(type);let path=c?.asset;if(type==='guard'||type==='prop')path=null;else if(type==='prop'&&(slotId==='M2'||slotId==='M4'))path=c.assetCCW;const g=normalizeComponentPalette((path?cloneAsset(path):null)||procedural(type,slotId),type);if(type==='fc')decoratePart(g,'fc',slotId);return g}
...
function buildDownwash(){
 sDownwash=[];sDownwashCones=[];sWashDiscs=[];const g=new THREE.Group(),centers=[[1.72,0,1.72],[-1.72,0,1.72],[-1.72,0,-1.72],[1.72,0,-1.72]];
 centers.forEach((c,mi)=>{
   const cone=new THREE.Mesh(new THREE.ConeGeometry(.66,2.3,32,1,true),new THREE.MeshBasicMaterial({color:0x8cdcf2,transparent:true,opacity:0,depthWrite:false,side:THREE.DoubleSide}));cone.position.set(c[0],-1.08,c[2]);cone.rotation.z=Math.PI;g.add(cone);sDownwashCones.push({cone,motor:mi});
   const disc=new THREE.Mesh(new THREE.CircleGeometry(.62,48),new THREE.MeshBasicMaterial({color:0xbfeeff,transparent:true,opacity:0,depthWrite:false,side:THREE.DoubleSide}));disc.rotation.x=-Math.PI/2;disc.position.set(c[0],-.14,c[2]);g.add(disc);sWashDiscs.push({disc,motor:mi});
   for(let j=0;j<8;j++){const ring=new THREE.Mesh(new THREE.TorusGeometry(.30,.012,8,48),new THREE.MeshBasicMaterial({color:0xc4efff,transparent:true,opacity:0,depthWrite:false}));ring.rotation.x=Math.PI/2;ring.position.set(c[0],-.20-j*.32,c[2]);g.add(ring);sDownwash.push({ring,motor:mi,phase:j/8})}
 });return g
}
...
function updateSimMotorBank(mix){
 if(!simMotorBank||!state.sim.running||!soundEnabled||soundVolume<=0)return;
 const t=audioCtx?.currentTime||0;
 mix.forEach((v,i)=>{
   const s=clamp(v/100,0,1),m=simMotorBank[i],hz=70+s*440+i*3;
   m.o.frequency.setTargetAtTime(hz,t,.035);m.f.frequency.setTargetAtTime(800+s*3000,t,.04);
   m.g.gain.setTargetAtTime((.0015+s*.0065)*soundVolume,t,.07)
 })
}
function silenceSimMotorBank(){
 if(!simMotorBank)return;const t=audioCtx?.currentTime||0;
 simMotorBank.forEach(m=>{try{m.g.gain.cancelScheduledValues(t);m.g.gain.setTargetAtTime(0,t,.008)}catch{}})
}
function destroySimMotorBank(){
 if(!simMotorBank)return;const t=audioCtx?.currentTime||0;
 simMotorBank.forEach(m=>{
   try{m.g.gain.cancelScheduledValues(t);m.g.gain.setValueAtTime(0,t);m.o.stop(t+.005)}catch{}
   try{m.o.disconnect();m.f.disconnect();m.g.disconnect()}catch{}
 });
 simMotorBank=null
}
function ensureSim(){
...
let simRealMirrorLast=0,simRealMirrorBusy=false,simRealMirrorConfirmed=false;
function simRealMirrorEnabled(){return !!$('#simRealMirror')?.checked}
function updateSimRealMirrorUi(msg=''){const on=simRealMirrorEnabled(),b=$('#simRealMirrorState');if(b){b.textContent=msg||(on?'REAL + SIM':'SIM ONLY');b.className='status '+(on?'good':'')}const cb=$('#simRealMirror');if(cb)cb.title=on?'Tripod RC/PID is mirrored to the selected real kit':'Simulator only'}
function simRealRcChannels(){const s=state.sim,keyRoll=(heldKeys.has(keyMap.rollRight)?1:0)-(heldKeys.has(keyMap.rollLeft)?1:0),keyPitch=(heldKeys.has(keyMap.pitchForward)?1:0)-(heldKeys.has(keyMap.pitchBack)?1:0),keyYaw=(heldKeys.has(keyMap.yawRight)?1:0)-(heldKeys.has(keyMap.yawLeft)?1:0),r=keyRoll||s.cmdRoll,p=keyPitch||s.cmdPitch,y=keyYaw||s.cmdYaw;return[Math.round(1500+clamp(r,-1,1)*500),Math.round(1500+clamp(p,-1,1)*500),Math.round(clamp(s.throttle,1000,2000)),Math.round(1500+clamp(y,-1,1)*500),s.running?2000:1000,s.flightMode==='rate'?1500:1000,1000,1000,1500,1000]}
async function setSimRealMirror(on){const cb=$('#simRealMirror');if(!cb)return false;if(!on){cb.checked=false;simRealMirrorConfirmed=false;updateSimRealMirrorUi();try{if(window.zebjusSchool?.canControl?.())await window.zebjusSchool.commandDevice?.({type:'rc_frame',channels:[1500,1500,1000,1500,1000,state.sim.flightMode==='rate'?1500:1000,1000,1000,1500,1000]})}catch{}return true}if(!window.zebjusSchool?.isKitActive?.()){cb.checked=false;updateSimRealMirrorUi('CONNECT KIT');notify('Connect the selected ZEBJUS kit before enabling real mirror.','bad');return false}if(!window.zebjusSchool?.canControl?.()){const ok=await window.zebjusSchool?.acquireLock?.(false);if(!ok){cb.checked=false;updateSimRealMirrorUi('TAKE CONTROL');notify('Take Control is required for Tripod → real kit mirror.','bad');return false}}if(!simRealMirrorConfirmed){if(!confirm('REAL KIT MIRROR can drive ESC/motor outputs. Secure the frame, verify motor order/direction, and use an appropriate safe bench/tether setup before continuing.')){cb.checked=false;updateSimRealMirrorUi();return false}simRealMirrorConfirmed=true}cb.checked=true;updateSimRealMirrorUi();notify('Tripod controls now mirror to the selected real FlightCore.','good');return true}
function simRealMirrorTick(now){if(!simRealMirrorEnabled()||simRealMirrorBusy||now-simRealMirrorLast<40)return;if(!window.zebjusSchool?.canControl?.()){const cb=$('#simRealMirror');if(cb)cb.checked=false;updateSimRealMirrorUi('CONTROL LOST');return}simRealMirrorLast=now;simRealMirrorBusy=true;window.zebjusSchool.commandDevice?.({type:'rc_frame',sequence:Math.floor(now),timestamp:Date.now(),channels:simRealRcChannels()}).catch(e=>{updateSimRealMirrorUi('RC ERROR');console.warn('[ZEBJUS] Tripod real mirror:',e)}).finally(()=>{simRealMirrorBusy=false})}
function mirrorSimPidToReal(){if(!simRealMirrorEnabled()||!window.zebjusSchool?.canControl?.())return;window.zebjusSchool.commandDevice?.({type:'pid_set',...pidCommandPayload(state.pid)}).then(()=>updateSimRealMirrorUi('PID SYNCED')).catch(e=>{updateSimRealMirrorUi('PID ERROR');notify(e?.message||'Could not sync PID to real kit.','bad')})}
function initSim(){
 ...
 sScene=new THREE.Scene();sScene.background=new THREE.Color(0x142535);sCamera=new THREE.PerspectiveCamera(46,w/h,.1,1000);sCamera.position.set(9,6.4,10.8);
 ...
 sScene.add(new THREE.AmbientLight(0xffffff,.86));sScene.add(new THREE.HemisphereLight(0xf1fbff,0x42586a,2.65));const l=new THREE.DirectionalLight(0xffffff,3.6);l.position.set(6,10,7);l.castShadow=true;sScene.add(l);const rim=new THREE.DirectionalLight(0x7ce4ff,1.45);rim.position.set(-6,5,-4);sScene.add(rim);const fill=new THREE.DirectionalLight(0xcfeaff,1.05);fill.position.set(-2,6,8);sScene.add(fill);const warm=new THREE.PointLight(0xffd6aa,1.45,20);warm.position.set(4,5,2);sScene.add(warm);
 ...
 const dustCount=180,pos=new Float32Array(dustCount*3);sDustBase=[];for(let i=0;i<dustCount;i++){const a=Math.random()*Math.PI*2,r=.8+Math.random()*4.2,y=.025+Math.random()*.23;pos[i*3]=Math.cos(a)*r;pos[i*3+1]=y;pos[i*3+2]=Math.sin(a)*r;sDustBase.push({a,r,y,seed:Math.random()*10})}
 const dg=new THREE.BufferGeometry();dg.setAttribute('position',new THREE.BufferAttribute(pos,3));const dm=new THREE.PointsMaterial({color:0xd9c29f,size:.055,transparent:true,opacity:0,depthWrite:false});sDust=new THREE.Points(dg,dm);sScene.add(sDust);
 ...
 $('#simRunBtn').onclick=toggleSimRun;$('#simResetBtn').onclick=resetSim;$('#simDisturbBtn').onclick=toggleDisturbMode;$('#simCalibrateLevelBtn').onclick=calibrateSimLevel;const simMirror=$('#simRealMirror');if(simMirror){simMirror.checked=false;simMirror.onchange=e=>setSimRealMirror(e.target.checked);updateSimRealMirrorUi()}$('#simFlightMode').onchange=e=>{state.sim.flightMode=e.target.value;if(state.sim.flightMode==='rate'){state.sim.rateHoldRoll=state.sim.roll;state.sim.rateHoldPitch=state.sim.pitch;state.sim.rateRollStickActive=false;state.sim.ratePitchStickActive=false}updateSimPidModeUI();syncQuickPid();resetSimControllersOnly();chart=[]};$('#simApplyPidBtn').onclick=applyQuickPid;
 initSticks();initManualDisturbance();initSimEnvironmentControls();initPidLearning();renderKeySettings();window.addEventListener('resize',resizeSim);window.addEventListener('keydown',simKeyDown);window.addEventListener('keyup',simKeyUp);window.addEventListener('blur',releaseSimDirectionalInput);document.addEventListener('visibilitychange',()=>{if(document.hidden)releaseSimDirectionalInput()});document.querySelectorAll('.tab[data-tab]').forEach(tab=>tab.addEventListener('click',()=>{if(tab.dataset.tab!=='sim')releaseSimDirectionalInput()}));requestAnimationFrame(simLoop)
}
...
function applyQuickPid(){
 const prefix=state.sim.flightMode==='angle'?'angleRate':'rate';[['Roll','simRateRoll'],['Pitch','simRatePitch'],['Yaw','simRateYaw']].forEach(([axis,p])=>Object.assign(state.pid[prefix+axis],{P:pidInput('#'+p+'P'),I:pidInput('#'+p+'I'),D:pidInput('#'+p+'D')}));[['angleRoll','simAngleRoll'],['anglePitch','simAnglePitch']].forEach(([k,p])=>Object.assign(state.pid[k],{P:pidInput('#'+p+'P'),I:pidInput('#'+p+'I'),D:pidInput('#'+p+'D')}));
 markPidDirty();resetSimControllersOnly();renderPid();updatePidCoach();mirrorSimPidToReal();notify(`${state.sim.flightMode==='rate'?'RATE-mode PID':'ANGLE outer + inner Rate PID'} applied${simRealMirrorEnabled()?' to Simulator + Real Kit':''}.`)
}
function syncQuickPid(){
 const prefix=state.sim.flightMode==='angle'?'angleRate':'rate',map=[[prefix+'Roll','simRateRoll'],[prefix+'Pitch','simRatePitch'],[prefix+'Yaw','simRateYaw'],['angleRoll','simAngleRoll'],['anglePitch','simAnglePitch']];
...
function stopSimRuntime(updateButton=true){
 state.sim.running=false;heldKeys.clear();if(simRealMirrorEnabled()&&window.zebjusSchool?.canControl?.())window.zebjusSchool.commandDevice?.({type:'rc_frame',channels:[1500,1500,1000,1500,1000,state.sim.flightMode==='rate'?1500:1000,1000,1000,1500,1000]}).catch(()=>{});
 destroyMotorAudio('sim');destroySimMotorBank();
 state.sim.motorActual=[0,0,0,0];state.sim.lastMix=[0,0,0,0];
 simPropBlurs.forEach(b=>{b.material.opacity=0;b.scale.setScalar(1)});
 sDownwash.forEach(x=>x.ring.material.opacity=0);sDownwashCones.forEach(x=>x.cone.material.opacity=0);sWashDiscs.forEach(x=>x.disc.material.opacity=0);
 if(sDust)sDust.material.opacity=0;
 if(updateButton&&$('#simRunBtn')){$('#simRunBtn').textContent='RUN';$('#simRunBtn').classList.remove('danger')}
}
function toggleSimRun(){
 if(window.__zebjusViewOnly)return;
 if(state.sim.running){stopSimRuntime(true);return}
 state.sim.running=true;$('#simRunBtn').textContent='STOP';$('#simRunBtn').classList.add('danger');
 ensureMotorAudio('sim');ensureSimMotorBank()
}
...
function resetSim(){
 stopSimRuntime(true);state.sim.throttle=1000;state.sim.disturbing=false;resetSimDynamics(true);
 setStickVisual('right',0,0);setStickVisual('left',0,1);updateStickText()
}
function resizeSim(){if(!sRenderer||!sCamera)return;const e=$('#simThree');if(!e)return;const r=e.getBoundingClientRect(),w=Math.max(1,Math.round(e.clientWidth||r.width||900)),h=Math.max(1,Math.round(e.clientHeight||r.height||650));sCamera.aspect=w/h;sCamera.updateProjectionMatrix();sRenderer.setSize(w,h,false)}
function avgPid(keys,v){return keys.reduce((s,k)=>s+state.pid[k][v],0)/keys.length}
function tuneQuality(){
 const s=state.sim,axis=s.teachAxis,k=activeRatePidKey(axis[0].toUpperCase()+axis.slice(1)),r=state.pid[k],angle=axis==='yaw'?null:state.pid['angle'+axis[0].toUpperCase()+axis.slice(1)];
 let score=0;
...
function rateAxis(axis,target,current,dt){
 const k=activeRatePidKey(axis),ik=axis.toLowerCase()+'I',pk='prev'+axis+'Err',err=target-current;
...
function initManualDisturbance(){
 ...
 c.addEventListener('pointerdown',e=>{if(window.__zebjusViewOnly||!state.sim.disturbMode)return;e.preventDefault();start={x:e.clientX,y:e.clientY,roll:state.sim.roll,pitch:state.sim.pitch};state.sim.disturbing=true;try{c.setPointerCapture(e.pointerId)}catch{}});
...
function simLoop(now){
 requestAnimationFrame(simLoop);const dt=Math.min(.035,(now-sLast)/1000||.016);sLast=now;const s=state.sim,q=tuneQuality();const remoteFollower=!!window.__zebjusRemoteFollower&&!!window.__zebjusRemoteSimState;if(remoteFollower){Object.assign(s,window.__zebjusRemoteSimState);heldKeys.clear()}const authority=clamp((s.throttle-1050)/650,0,1);
 let uR=0,uP=0,uY=0,cmdR=s.cmdRoll,cmdP=s.cmdPitch,cmdY=s.cmdYaw;
 if(s.running&&!remoteFollower){
   const keyRoll=(heldKeys.has(keyMap.rollRight)?1:0)-(heldKeys.has(keyMap.rollLeft)?1:0);
   const keyPitch=(heldKeys.has(keyMap.pitchForward)?1:0)-(heldKeys.has(keyMap.pitchBack)?1:0);
   const keyYaw=(heldKeys.has(keyMap.yawRight)?1:0)-(heldKeys.has(keyMap.yawLeft)?1:0);
   cmdR=keyRoll!==0?keyRoll:s.cmdRoll;cmdP=keyPitch!==0?keyPitch:s.cmdPitch;cmdY=keyYaw!==0?keyYaw:s.cmdYaw;
   setStickVisual('right',cmdR,-cmdP);
   setStickVisual('left',cmdY,(1500-s.throttle)/500);
   let targetRR=0,targetPR=0,targetYR=cmdY*180;s.targetYawRate=targetYR;
   if(s.flightMode==='angle'){
     s.targetRoll=s.levelTrimRoll+cmdR*26;
     s.targetPitch=s.levelTrimPitch+cmdP*26;
     targetRR=outerAxis('Roll',s.targetRoll,s.roll,dt);
     targetPR=outerAxis('Pitch',s.targetPitch,s.pitch,dt)
   }else{
     targetRR=rateHoldTargetRate('Roll',cmdR,s.roll);
     targetPR=rateHoldTargetRate('Pitch',cmdP,s.pitch);
 ...
 }else if(!remoteFollower){s.rollRate*=Math.exp(-2*dt);s.pitchRate*=Math.exp(-2*dt);s.yawRate*=Math.exp(-2*dt)}

 const vib=s.running?q.vib*(.25+.75*authority):0,jr=Math.sin(now*.052)*vib,jp=Math.sin(now*.071+1.7)*vib*.82;
 // Explicit body-axis mapping: Roll = front/back Z axis, Pitch = left/right X axis, Yaw = vertical Y axis.
 sPivot.rotation.set(rad(s.pitch+jp),rad(s.yaw),-rad(s.roll+jr),'YXZ');
 ...
 const benchMix=pythonBenchMix(now);let mix;
 if(benchMix){mix=benchMix;s.motorActual=[...mix];s.lastMix=[...mix]}else if(remoteFollower){mix=(Array.isArray(s.lastMix)?s.lastMix:[0,0,0,0]).map(v=>clamp(+v||0,0,100));s.motorActual=[...mix];s.lastMix=[...mix]}else{const base=s.running?clamp((s.throttle-1000)/10,0,100):0,rc=s.running?clamp(uR*.058,-36,36):0,pc=s.running?clamp(uP*.058,-36,36):0,yc=s.running?clamp(uY*.038,-26,26):0;const cmdMix=s.running?[base+rc-pc+yc,base-rc-pc-yc,base-rc+pc+yc,base+rc+pc-yc].map(v=>clamp(v,0,100)):[0,0,0,0];const lag=Math.max(.03,s.motorLag),aLag=s.running?1-Math.exp(-dt/lag):1;s.motorActual=s.motorActual.map((v,i)=>s.running?v+(cmdMix[i]-v)*aLag:0);mix=s.motorActual;s.lastMix=mix}

 simProps.forEach((p,i)=>{const sp=mix[i]/100;p.rotation.y+=(i%2?1:-1)*dt*(1+sp*138)});
 simPropBlurs.forEach((b,i)=>{const sp=mix[i]/100;b.material.opacity=clamp((sp-.05)*.52,0,.46);b.scale.setScalar(1+sp*.16)});
 ...
 const groundFactor=clamp(1-Math.max(0,s.liftY)/.62,.14,1);
 sDownwash.forEach(x=>{const sp=mix[x.motor]/100,t=(now*.00034*(.38+sp*4.8)+x.phase)%1;x.ring.position.y=-.18-t*(2.55+sp*1.15);x.ring.scale.setScalar(.72+t*(.96+sp*.72));x.ring.material.opacity=s.running?clamp(sp*.46*(1-t)*groundFactor,0,.40):0});
 sDownwashCones.forEach(x=>{const sp=mix[x.motor]/100;x.cone.material.opacity=s.running?sp*.11*groundFactor:0;x.cone.scale.set(1+sp*.62,1+sp*.42,1+sp*.62)});
 sWashDiscs.forEach(x=>{const sp=mix[x.motor]/100;x.disc.material.opacity=s.running?clamp(.05+sp*.18,0,.22)*groundFactor:0;x.disc.scale.setScalar(1+sp*.85)});
 if(sDust){const ar=sDust.geometry.attributes.position.array;sDust.material.opacity=s.running?clamp((avg-.10)*.58*groundFactor,0,.34):0;for(let i=0;i<sDustBase.length;i++){const d=sDustBase[i],flow=(now*.000075*(.35+avg*2.2)+d.seed)%1,rr=d.r+flow*(.55+avg*1.45)*groundFactor;ar[i*3]=Math.cos(d.a)*rr;ar[i*3+2]=Math.sin(d.a)*rr;ar[i*3+1]=d.y+Math.abs(Math.sin(flow*Math.PI))*avg*.18*groundFactor}sDust.geometry.attributes.position.needsUpdate=true}
 ...
 updateStickText();simRealMirrorTick(now);sControls.update();sRenderer.render(sScene,sCamera);
...
function applyPidPreset(name){
 const stable=normalizePidShape({}),p=JSON.parse(JSON.stringify(stable));
...
function pidLessonDiagnosis(){
 const axis=state.sim.teachAxis,k=activeRatePidKey(axis[0].toUpperCase()+axis.slice(1)),r=state.pid[k],a=axis==='yaw'?null:state.pid['angle'+axis[0].toUpperCase()+axis.slice(1)],notes=[];let label='STABLE RANGE';
...
function simKeyDown(e){if(window.__zebjusViewOnly)return;if(keyCaptureAction){e.preventDefault();const key=e.key.length===1?e.key.toLowerCase():e.key;if(['t','m','x'].includes(key)||Object.entries(keyMap).some(([action,value])=>action!==keyCaptureAction&&value===key)){notify('This key is reserved or already assigned. Choose another.','bad');return}keyMap[keyCaptureAction]=key;keyCaptureAction=null;heldKeys.clear();renderKeySettings();window.dispatchEvent(new Event('zebjus-keymap-change'));return}if(/INPUT|TEXTAREA|SELECT/.test(document.activeElement?.tagName||''))return;if(!$('#tab-sim')?.classList.contains('active'))return;const key=e.key.length===1?e.key.toLowerCase():e.key;if(!Object.values(keyMap).includes(key))return;e.preventDefault();if(key===keyMap.run){if(!e.repeat)toggleSimRun();return}heldKeys.add(key);if(key===keyMap.throttleUp)state.sim.throttle=clamp(state.sim.throttle+25,1000,2000);if(key===keyMap.throttleDown)state.sim.throttle=clamp(state.sim.throttle-25,1000,2000);const c=simEffectiveKeys();setStickVisual('right',c.roll,-c.pitch);setStickVisual('left',c.yaw,(1500-state.sim.throttle)/500);updateStickText()}
function simKeyUp(e){const key=e.key.length===1?e.key.toLowerCase():e.key;heldKeys.delete(key);if(!Object.values(keyMap).includes(key))return;const c=simEffectiveKeys();setStickVisual('right',c.roll,-c.pitch);setStickVisual('left',c.yaw,(1500-state.sim.throttle)/500);updateStickText()}
function renderKeySettings(){const box=$('#keySettings');if(!box)return;const labels={rollLeft:'Roll left',rollRight:'Roll right',pitchForward:'Pitch forward',pitchBack:'Pitch back',throttleUp:'Throttle +',throttleDown:'Throttle −',yawLeft:'Yaw CCW',yawRight:'Yaw CW',run:'Simulator Run / Stop'};box.innerHTML=Object.entries(labels).map(([k,l])=>`<div class="key-row"><span>${l}</span><button class="key-capture ${keyCaptureAction===k?'listening':''}" data-key-action="${k}">${keyCaptureAction===k?'PRESS KEY…':keyLabel(keyMap[k])}</button></div>`).join('');$$('.key-capture').forEach(b=>b.onclick=()=>{keyCaptureAction=b.dataset.keyAction;renderKeySettings()});window.dispatchEvent(new Event('zebjus-keymap-change'))}
...
function projectPayload(){return{version:'18.3.46',savedAt:new Date().toISOString(),guided:state.guided,step:state.step,parts:state.parts.filter(p=>!p.internal&&p.type!=='batteryStrap').map(p=>({type:p.type,slotId:p.slotId})),actions:[...state.doneActions],connections:state.connections,pid:state.pid,wireLayout,wireNodeTransforms,optionalWireNodes,sim:{batteryV:state.sim.batteryV,payloadG:state.sim.payloadG,cgX:state.sim.cgX,cgY:state.sim.cgY,wind:state.sim.wind,motorLag:state.sim.motorLag}}}
...
function exportDiagnostics(){const selected=window.zebjusSchool?.getSelectedDevice?.()||null,d={generatedAt:new Date().toISOString(),version:'18.3.46',runtime:{viewport:[innerWidth,innerHeight],pixelRatio:devicePixelRatio||1,parts:state.parts.filter(p=>!p.internal).length,connections:state.connections.length,assemblyStep:state.step,simMode:state.sim.flightMode},kit:selected?{deviceName:selected.deviceName,deviceId:selected.deviceId,boardId:selected.boardId,boardName:selected.boardName,firmware:selected.firmware||selected.version,firmwareRole:selected.firmwareRole,flightCoreIntegrated:!!selected.flightCoreIntegrated,imuModel:selected.imuModel,receiverHealth:selected.receiverHealth,online:!!selected.online,verifiedConnected:!!window.zebjusSchool?.isSelectedConnected?.()}:null,python:{files:Object.keys(pythonFiles),active:pythonActiveFile,running:pythonRunning,imuSamples:imuToolSamples.length},electrical:electricalIssues()};downloadBlob('ZEBJUS_Diagnostics.json',JSON.stringify(d,null,2),'application/json')}
...
function registerOffline(){
 ...
 let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;if(pythonRunning||state.sim?.running){const tag=$('#assetStatus');if(tag)tag.textContent='Update ready • reload after stopping flight';return}location.reload()});
 navigator.serviceWorker.register('./service-worker.js?v=18.3.46',{updateViaCache:'none'}).then(reg=>reg.update().catch(()=>{})).catch(()=>{});
}
...
function initAudioSettings(){
 updateAudioUi();
 if($('#soundEnabled'))$('#soundEnabled').onchange=e=>{soundEnabled=e.target.checked;try{localStorage.setItem('zebjus-v171-sound',soundEnabled?'on':'off')}catch{};if(!soundEnabled){stopTransientTones();Object.keys(motorAudio).forEach(destroyMotorAudio);destroySimMotorBank?.()}else tone(660,.06,'sine',.008);updateAudioUi()};
...
function packet(raw){let d,text;if(typeof raw==='string'){text=raw;try{d=JSON.parse(raw)}catch{d={raw}}}else{d=raw||{};text=JSON.stringify(d)};['roll','pitch','yaw','gyroX','gyroY','gyroZ','battery'].forEach(k=>{if(Number.isFinite(+d[k]))state.telemetry[k]=+d[k]});if(Number.isFinite(+d.accX)&&Number.isFinite(+d.gyroX))recordImuSample?.(d);if(d?.pid&&typeof d.pid==='object'){state.pid=normalizePidShape(JSON.parse(JSON.stringify(d.pid)));renderPid();syncQuickPid?.();updatePidCoach?.()}if(d?.type==='ack'&&d?.command==='pid_set'){pidDirty=false;updatePidSaveState?.('saved')}$('#telemetryLog').textContent=(new Date().toLocaleTimeString()+' '+text+'\n'+$('#telemetryLog').textContent).slice(0,12000);telemetryUI()}
...
function calibrationUi(c){
 if(!c)return;const a=c.accelOffset||{},g=c.gyroBias||{},t=c.levelTrim||{},live=c.accel||{};
 const set=(id,v,d=3)=>{const e=$(id);if(e&&Number.isFinite(+v))e.value=(+v).toFixed(d)};
 set('#calAccX',a.x,4);set('#calAccY',a.y,4);set('#calAccZ',a.z,4);set('#calTrimRoll',t.roll,2);set('#calTrimPitch',t.pitch,2);
 const txt=(id,v,d=3,suffix='')=>{const e=$(id);if(e)e.textContent=Number.isFinite(+v)?(+v).toFixed(d)+suffix:'--'};
 txt('#calBiasRoll',g.roll,3,' dps');txt('#calBiasPitch',g.pitch,3,' dps');txt('#calBiasYaw',g.yaw,3,' dps');txt('#calLiveAx',live.x,3,' g');txt('#calLiveAy',live.y,3,' g');txt('#calLiveAz',live.z,3,' g');
}
async function calibrationRealCommand(type,data={}){
 if(!window.zebjusSchool?.isKitActive?.())throw new Error('Connect the selected ZEBJUS kit first.');
 const r=await window.zebjusSchool.commandDevice({type,...data});if(r?.calibration)calibrationUi(r.calibration);return r
}
async function readCalibrationUi(){const b=$('#calSaveBadge'),n=$('#calNote');try{if(b){b.textContent='READING';b.className='status'}const r=await calibrationRealCommand('calibration_get');if(b){b.textContent='LOADED';b.className='status good'}if(n)n.textContent='Calibration loaded from the selected Device ID. Read-only: no Take Control required.';return r}catch(e){if(b){b.textContent='ERROR';b.className='status bad'}if(n)n.textContent=e.message;notify(e.message,'bad')}}
async function saveCalibrationUi(){const v=id=>Number($(id)?.value);const data={accelOffsetX:v('#calAccX'),accelOffsetY:v('#calAccY'),accelOffsetZ:v('#calAccZ'),levelTrimRoll:v('#calTrimRoll'),levelTrimPitch:v('#calTrimPitch')},b=$('#calSaveBadge'),n=$('#calNote');try{if(b){b.textContent='SAVING';b.className='status'}const r=await calibrationRealCommand('calibration_set',data);if(b){b.textContent='SAVED';b.className='status good'}if(n)n.textContent='X/Y/Z offsets and level trim saved to FlightCore NVS. Python/Web commands automatically acquired control.';notify('FlightCore level offsets saved.','good');return r}catch(e){if(b){b.textContent='ERROR';b.className='status bad'}if(n)n.textContent=e.message;notify(e.message,'bad')}}
async function captureLevelUi(){const b=$('#calSaveBadge'),n=$('#calNote');if(!confirm('REMOVE PROPELLERS. Place the drone LEVEL and COMPLETELY STILL, then capture X=0 g, Y=0 g, Z=+1 g offsets?'))return;try{if(b){b.textContent='CAPTURING';b.className='status'}const r=await calibrationRealCommand('level_calibrate',{samples:400});if(b){b.textContent='SAVED';b.className='status good'}if(n)n.textContent=`Level captured from ${r?.samples||400} samples. Accelerometer offsets are persistent; gyro bias is refreshed from the still sensor.`;notify('Level calibration captured and saved.','good');return r}catch(e){if(b){b.textContent='ERROR';b.className='status bad'}if(n)n.textContent=e.message;notify(e.message,'bad')}}
async function restoreCalibrationUi(){if(!confirm('Restore factory level offsets (-0.10, +0.03, +0.12 g) and zero Roll/Pitch trim?'))return;try{const r=await calibrationRealCommand('calibration_defaults');const b=$('#calSaveBadge');if(b){b.textContent='DEFAULTS';b.className='status good'}notify('Calibration defaults restored.','good');return r}catch(e){notify(e.message,'bad')}}
function initCal(){
 const a=[['Gyro zero','Keep level and still','calibrate_gyro'],['Auto level','Capture X/Y/Z level offsets','level_calibrate'],['Motor order','REMOVE PROPELLERS','motor_order_test']];$('#calSteps').innerHTML=a.map((v,i)=>`<div class="cal-step"><i>${i+1}</i><div><b>${v[0]}</b><span>${v[1]}</span></div><button class="btn ghost small" data-c="${v[2]}">Run</button></div>`).join('');
 $$('#calSteps button').forEach(b=>b.onclick=async()=>{const type=b.dataset.c;if(type==='motor_order_test'){if(!confirm('REMOVE ALL PROPELLERS and secure the frame before motor-order testing. Continue?'))return;try{await calibrationRealCommand(type,{confirm:'PROPS_REMOVED'});notify('Motor order test started.','good')}catch(e){notify(e.message,'bad')}return}if(type==='level_calibrate'){captureLevelUi();return}try{await calibrationRealCommand(type);notify(type==='calibrate_gyro'?'Gyro calibration complete.':'Calibration command complete.','good');await readCalibrationUi()}catch(e){notify(e.message,'bad')}});
 if($('#calReadBtn'))$('#calReadBtn').onclick=readCalibrationUi;if($('#calSaveBtn'))$('#calSaveBtn').onclick=saveCalibrationUi;if($('#calCaptureBtn'))$('#calCaptureBtn').onclick=captureLevelUi;if($('#calDefaultsBtn'))$('#calDefaultsBtn').onclick=restoreCalibrationUi;
}
function markPidDirty(){pidDirty=true;updatePidSaveState('dirty')}
function setSimFlightModeExternal(mode){ensureSim();const wanted=String(mode||'').toLowerCase(),next=wanted==='rate'?'rate':'angle';if(state.sim.flightMode===next)return true;state.sim.flightMode=next;if(next==='rate'){state.sim.rateHoldRoll=state.sim.roll;state.sim.rateHoldPitch=state.sim.pitch;state.sim.rateRollStickActive=false;state.sim.ratePitchStickActive=false}const sel=$('#simFlightMode');if(sel)sel.value=next;updateSimPidModeUI();syncQuickPid();resetSimControllersOnly();chart=[];return true}
...
function pidCommandPayload(pid=state.pid){const p=normalizePidShape(JSON.parse(JSON.stringify(pid||{}))),out={};for(const key of['rateRoll','ratePitch','rateYaw','angleRateRoll','angleRatePitch','angleRateYaw','angleRoll','anglePitch'])for(const k of['P','I','D'])out[key+k]=Number(p[key][k]);return out}
function applyPidFlat(data){const p=normalizePidShape(JSON.parse(JSON.stringify(state.pid)));for(const key of['rateRoll','ratePitch','rateYaw','angleRateRoll','angleRatePitch','angleRateYaw','angleRoll','anglePitch'])for(const k of['P','I','D'])if(Number.isFinite(Number(data?.[key+k])))p[key][k]=Number(data[key+k]);state.pid=p;renderPid();syncQuickPid();updatePidCoach?.();resetSimControllersOnly();return p}
function simReceiverChannels(){const s=state.sim;return[Math.round(1500+s.cmdRoll*500),Math.round(1500+s.cmdPitch*500),Math.round(s.throttle),Math.round(1500+s.cmdYaw*500),s.running?2000:1000,s.flightMode==='rate'?1500:1000,1000,1000,1500,1000]}
function pythonBenchMix(now=performance.now()){const b=state.sim.pythonBench;if(!b)return null;const elapsed=now-b.started;if(b.mode==='motor'){if(elapsed>=b.duration){state.sim.pythonBench=null;return[0,0,0,0]}const x=[0,0,0,0];x[Math.max(0,Math.min(3,b.motor-1))]=clamp((b.pulse-1000)/10,0,100);return x}if(b.mode==='sequence'){const m=Math.floor(elapsed/900);if(m>=4){state.sim.pythonBench=null;return[0,0,0,0]}const x=[0,0,0,0];x[m]=20;return x}if(b.mode==='esc'){if(elapsed>=6000){state.sim.pythonBench=null;return[0,0,0,0]}return elapsed<3000?[100,100,100,100]:[2.4,2.4,2.4,2.4]}return null}
async function pythonSimCommand(type,data={}){ensureSim();if(type==='pid_get')return{ok:true,type:'ack',command:type,pid:JSON.parse(JSON.stringify(state.pid))};if(type==='pid_set'){const pid=applyPidFlat(data);markPidDirty();return{ok:true,type:'ack',command:type,saved:false,simulator:true,pid}}if(type==='pid_defaults'){state.pid=normalizePidShape({});renderPid();syncQuickPid();resetSimControllersOnly();return{ok:true,type:'ack',command:type,simulator:true,pid:JSON.parse(JSON.stringify(state.pid))}}if(type==='attitude_read')return{ok:true,type:'ack',command:type,roll:state.sim.roll,pitch:state.sim.pitch,yaw:state.sim.yaw,rateRoll:state.sim.rollRate,ratePitch:state.sim.pitchRate,rateYaw:state.sim.yawRate};if(type==='receiver_read'||type==='ppm_read')return{ok:true,type:'ack',command:'receiver_read',source:'SIM',ppmFresh:false,channels:simReceiverChannels()};if(type==='rc_frame'){let ch=data.channels;if(typeof ch==='string')ch=ch.split(',').map(Number);if(!Array.isArray(ch)||ch.length<6)throw new Error('rc_frame requires CH1..CH6');setSimFlightModeExternal(Number(ch[5])>=1500?'rate':'angle');state.sim.cmdRoll=clamp((Number(ch[0])-1500)/500,-1,1);state.sim.cmdPitch=clamp((Number(ch[1])-1500)/500,-1,1);state.sim.throttle=clamp(Number(ch[2])||1000,1000,2000);state.sim.cmdYaw=clamp((Number(ch[3])-1500)/500,-1,1);setStickVisual('right',state.sim.cmdRoll,-state.sim.cmdPitch);setStickVisual('left',state.sim.cmdYaw,(1500-state.sim.throttle)/500);updateStickText();const arm=Number(ch[4])>=1500;if(arm&&!state.sim.running){state.sim.running=true;if($('#simRunBtn')){$('#simRunBtn').textContent='STOP';$('#simRunBtn').classList.add('danger')}ensureMotorAudio('sim');ensureSimMotorBank()}if(!arm&&state.sim.running)stopSimRuntime(true);return{ok:true,type:'ack',command:type,activeSource:'SIM',channels:ch.map(Number)}}if(type==='motor_test'){state.sim.running=false;state.sim.pythonBench={mode:'motor',started:performance.now(),motor:Math.max(1,Math.min(4,Number(data.motor)||1)),pulse:clamp(Number(data.pulse)||1200,1050,1300),duration:clamp(Number(data.durationMs)||700,100,3000)};return{ok:true,type:'ack',command:type,simulator:true,...state.sim.pythonBench}}if(type==='motor_order_test'){state.sim.running=false;state.sim.pythonBench={mode:'sequence',started:performance.now()};return{ok:true,type:'ack',command:type,simulator:true}}if(type==='esc_calibrate'){state.sim.running=false;state.sim.pythonBench={mode:'esc',started:performance.now()};return{ok:true,type:'ack',command:type,simulator:true,message:'Simulated 3 s high + 3 s low ESC calibration'}}if(type==='motor_stop'){state.sim.pythonBench=null;state.sim.motorActual=[0,0,0,0];state.sim.lastMix=[0,0,0,0];return{ok:true,type:'ack',command:type}}if(type==='bench_status')return{ok:true,type:'ack',command:type,benchMode:state.sim.pythonBench?.mode||'NONE'};if(type==='calibration_get'){state.sim.calibration=state.sim.calibration||{accelOffset:{x:0,y:0,z:0},gyroBias:{roll:0,pitch:0,yaw:0},levelTrim:{roll:state.sim.levelTrimRoll||0,pitch:state.sim.levelTrimPitch||0}};state.sim.calibration.levelTrim={roll:state.sim.levelTrimRoll||0,pitch:state.sim.levelTrimPitch||0};return{ok:true,type:'ack',command:type,simulator:true,calibration:JSON.parse(JSON.stringify(state.sim.calibration))}}if(type==='calibration_set'){state.sim.calibration=state.sim.calibration||{accelOffset:{x:0,y:0,z:0},gyroBias:{roll:0,pitch:0,yaw:0},levelTrim:{roll:0,pitch:0}};state.sim.calibration.accelOffset={x:Number(data.accelOffsetX)||0,y:Number(data.accelOffsetY)||0,z:Number(data.accelOffsetZ)||0};state.sim.levelTrimRoll=Number(data.levelTrimRoll)||0;state.sim.levelTrimPitch=Number(data.levelTrimPitch)||0;state.sim.calibration.levelTrim={roll:state.sim.levelTrimRoll,pitch:state.sim.levelTrimPitch};resetSimDynamics(true);return{ok:true,type:'ack',command:type,simulator:true,saved:false,calibration:JSON.parse(JSON.stringify(state.sim.calibration))}}if(type==='level_calibrate'||type==='calibrate_level'){calibrateSimLevel();state.sim.calibration=state.sim.calibration||{accelOffset:{x:0,y:0,z:0},gyroBias:{roll:0,pitch:0,yaw:0},levelTrim:{roll:0,pitch:0}};state.sim.calibration.levelTrim={roll:state.sim.levelTrimRoll||0,pitch:state.sim.levelTrimPitch||0};return{ok:true,type:'ack',command:'level_calibrate',simulator:true,calibration:JSON.parse(JSON.stringify(state.sim.calibration)),message:'Simulator level captured'}}if(type==='calibration_defaults'){state.sim.calibration={accelOffset:{x:-0.10,y:0.03,z:0.12},gyroBias:{roll:0,pitch:0,yaw:0},levelTrim:{roll:0,pitch:0}};state.sim.levelTrimRoll=0;state.sim.levelTrimPitch=0;resetSimDynamics(true);return{ok:true,type:'ack',command:type,simulator:true,calibration:JSON.parse(JSON.stringify(state.sim.calibration))}}if(type==='calibrate_gyro'){calibrateSimLevel?.();resetSimControllersOnly();return{ok:true,type:'ack',command:type,simulator:true,message:'Simulator level/gyro reference reset'}}if(type==='ping')return{ok:true,type:'ack',command:type,message:'PONG from Simulator'};throw new Error('Simulator command not supported: '+type)}
function pythonAssignNumber(code,name){const escaped=name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),m=String(code||'').match(new RegExp(`(?:^|\\n)\\s*${escaped}\\s*=\\s*([-+]?\\d+(?:\\.\\d+)?)\\b`,'m'));return m?Number(m[1]):null}
function syncPythonCodeToSimulator(code){const c=String(code||''),angleCode=/\bPAngle(?:Roll|Pitch)\b/.test(c),prefix=angleCode?'angleRate':'rate',map=[['Roll','Roll'],['Pitch','Pitch'],['Yaw','Yaw']];let changed=false;for(const [axis,varAxis] of map){for(const k of['P','I','D']){const v=pythonAssignNumber(c,`${k}Rate${varAxis}`);if(v!=null&&Number.isFinite(v)){state.pid[prefix+axis][k]=v;changed=true}}}for(const axis of['Roll','Pitch'])for(const k of['P','I','D']){const v=pythonAssignNumber(c,`${k}Angle${axis}`);if(v!=null&&Number.isFinite(v)){state.pid['angle'+axis][k]=v;changed=true}}const modeMatch=c.match(/(?:flight_mode|mode)\s*=\s*['\"](angle|rate)['\"]/i);if(modeMatch)setSimFlightModeExternal(modeMatch[1]);const t=pythonAssignNumber(c,'InputThrottle')??pythonAssignNumber(c,'throttle');if(t!=null){state.sim.throttle=clamp(t,1000,2000);setStickVisual('left',state.sim.cmdYaw,(1500-state.sim.throttle)/500);updateStickText()}if(changed){renderPid();syncQuickPid();updatePidCoach?.();resetSimControllersOnly();return true}return false}
function renderPid(){
 state.pid=normalizePidShape(state.pid);
 const g=[['RATE mode PID',[['Roll','rateRoll'],['Pitch','ratePitch'],['Yaw','rateYaw']]],['ANGLE mode inner Rate PID',[['Roll','angleRateRoll'],['Pitch','angleRatePitch'],['Yaw','angleRateYaw']]],['ANGLE outer PID • Roll/Pitch',[['Roll','angleRoll'],['Pitch','anglePitch']]]];
...
let py=null,pyodidePromise=null,pythonMonaco=null,monacoPromise=null,pythonPreludeReady=false,pythonStopRequested=false,pythonRunning=false,pythonStderrBuffer='',pythonHasRun=false,pythonWorker=null,pythonWorkerRunId=0;const pythonHeldKeys=new Set();
let imuToolSamples=[],imuCsvRows=[],imuRecording=false,imuLastAt=0,imuPrevAt=0,imuCal={ax:0,ay:0,az:0,gx:0,gy:0,gz:0};
try{Object.assign(imuCal,JSON.parse(localStorage.getItem('zebjus-imu-teaching-cal')||'{}')||{})}catch{}
...
const PY_IMU_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("IMU live stream. Press Stop to end.\\n")\nwhile not drone.stop_requested():\n    imu = await drone.imu()\n    a, g = imu["accel"], imu["gyro"]\n    print(f"ACC  X={a['x']:+.3f} Y={a['y']:+.3f} Z={a['z']:+.3f} g")\n    print(f"GYRO X={g['x']:+.2f} Y={g['y']:+.2f} Z={g['z']:+.2f} dps")\n    await asyncio.sleep(0.12)\n`;
const PY_GYRO_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nwhile not drone.stop_requested():\n    g = await drone.gyro()\n    print(f"Roll={g['x']:+7.2f}  Pitch={g['y']:+7.2f}  Yaw={g['z']:+7.2f} °/s")\n    await asyncio.sleep(0.10)\n`;
const PY_ACCEL_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nwhile not drone.stop_requested():\n    a = await drone.accel()\n    print(f"X={a['x']:+.3f} g  Y={a['y']:+.3f} g  Z={a['z']:+.3f} g")\n    await asyncio.sleep(0.10)\n`;
const PY_ANGLE_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("Real FlightCore attitude • move/tilt the MPU6050 / frame")\nwhile not drone.stop_requested():\n    a = await drone.attitude()\n    print(f"Roll={a['roll']:+.2f}°  Pitch={a['pitch']:+.2f}°  Yaw={a['yaw']:+.2f}°  age={a.get('sampleAgeMs',-1)}ms")\n    await asyncio.sleep(0.10)\n`;
const PY_PPM_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nwhile not drone.stop_requested():\n    rx = await drone.receiver()\n    ch = rx["channels"]\n    print("Source:", rx.get("source"), " | ", " ".join(f"CH{i+1}={v}" for i,v in enumerate(ch[:6])))\n    await asyncio.sleep(0.12)\n`;
const PY_PID_READ_EXAMPLE=`from zebjus import Drone\nimport json\n\ndrone = Drone()\nr = await drone.pid_get()\nprint(json.dumps(r["pid"], indent=2))\n`;
const PY_RATE_PID_EXAMPLE=`from zebjus import Drone\n\ndrone = Drone()\n# RATE mode values. On Simulator target these values immediately update the Tripod PID model.\nPRateRoll = 1.40\nIRateRoll = 18.0\nDRateRoll = 0.035\nPRatePitch = 1.40\nIRatePitch = 18.0\nDRatePitch = 0.035\nPRateYaw = 3.0\nIRateYaw = 20.0\nDRateYaw = 0.0\n\nr = await drone.set_rate_pid(\n    roll=(PRateRoll, IRateRoll, DRateRoll),\n    pitch=(PRatePitch, IRatePitch, DRatePitch),\n    yaw=(PRateYaw, IRateYaw, DRateYaw))\nprint("RATE PID applied:", r)\n`;
const PY_ANGLE_PID_EXAMPLE=`from zebjus import Drone\n\ndrone = Drone()\n# ANGLE outer loop + ANGLE-mode inner Rate loop.\nPAngleRoll = 3.0\nIAngleRoll = 0.0\nDAngleRoll = 0.0\nPAnglePitch = 3.0\nIAnglePitch = 0.0\nDAnglePitch = 0.0\nPRateRoll = 0.90\nIRateRoll = 15.0\nDRateRoll = 0.030\nPRatePitch = 0.90\nIRatePitch = 15.0\nDRatePitch = 0.030\nPRateYaw = 3.0\nIRateYaw = 15.0\nDRateYaw = 0.0\n\nr = await drone.set_angle_pid(\n    outer_roll=(PAngleRoll, IAngleRoll, DAngleRoll),\n    outer_pitch=(PAnglePitch, IAnglePitch, DAnglePitch),\n    inner_roll=(PRateRoll, IRateRoll, DRateRoll),\n    inner_pitch=(PRatePitch, IRatePitch, DRatePitch),\n    inner_yaw=(PRateYaw, IRateYaw, DRateYaw))\nprint("ANGLE PID applied:", r)\n`;
const PY_KEYBOARD_RC_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\n# Edit these keys in this file. The Joystick page opens while this runs.\nKEY_THROTTLE_UP = "w"\nKEY_THROTTLE_DOWN = "s"\nKEY_ROLL_LEFT = "ArrowLeft"\nKEY_ROLL_RIGHT = "ArrowRight"\nKEY_PITCH_FORWARD = "ArrowUp"\nKEY_PITCH_BACK = "ArrowDown"\nKEY_YAW_CCW = "a"\nKEY_YAW_CW = "d"\nKEY_ARM = "r"\nKEY_DISARM = "x"\nKEY_MODE = "m"\n\nthrottle = 1000\narmed = False\nmode = "angle"\nlast_mode_key = False\nframe_number = 0\nprint("Keyboard TX • W/S throttle • arrows roll/pitch • A CCW / D CW • R ARM • X DISARM • M mode")\nprint("Set Real ZEBJUS kit target and connect the exact Device ID before flight control.")\nwhile True:\n    keys = set(await drone.keys())\n    if KEY_DISARM in keys:\n        armed = False\n        throttle = 1000\n    elif KEY_ARM in keys and throttle <= 1050:\n        armed = True\n    if KEY_THROTTLE_UP in keys: throttle = min(1800, throttle + 8)\n    if KEY_THROTTLE_DOWN in keys: throttle = max(1000, throttle - 12)\n    roll = 1500 + (300 if KEY_ROLL_RIGHT in keys else -300 if KEY_ROLL_LEFT in keys else 0)\n    pitch = 1500 + (300 if KEY_PITCH_FORWARD in keys else -300 if KEY_PITCH_BACK in keys else 0)\n    yaw = 1500 + (250 if KEY_YAW_CW in keys else -250 if KEY_YAW_CCW in keys else 0)\n    toggle = KEY_MODE in keys\n    if toggle and not last_mode_key:\n        mode = "rate" if mode == "angle" else "angle"\n        armed = False\n        throttle = 1000\n        print("Mode changed • DISARMED, throttle 1000")\n    last_mode_key = toggle\n    ack = await drone.rc(roll=roll, pitch=pitch, throttle=throttle, yaw=yaw, arm=armed, mode=mode)\n    frame_number += 1\n    if frame_number % 5 == 0:\n        print(f"TX #{frame_number} R={roll} P={pitch} T={throttle} Y={yaw} ARM={armed} MODE={mode} FC={ack.get('activeSource','--')}")\n    await asyncio.sleep(0.04)\n`;
const PY_RC_SCRIPT_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("Scripted transmitter demo • Simulator recommended")\nawait drone.rc(throttle=1000, arm=False, mode="angle")\nfor throttle in range(1000, 1451, 10):\n    await drone.rc(throttle=throttle, roll=1500, pitch=1500, yaw=1500, arm=True, mode="angle")\n    await asyncio.sleep(0.04)\nfor roll in range(1500, 1701, 10):\n    await drone.rc(throttle=1450, roll=roll, arm=True, mode="angle")\n    await asyncio.sleep(0.04)\nawait drone.rc(throttle=1000, arm=False, mode="angle")\nprint("Demo complete")\n`;
const PY_MOTOR_TEST_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("REMOVE ALL PROPELLERS before a real motor test.")\nfor motor in range(1,5):\n    print("Testing motor", motor)\n    print(await drone.motor_test(motor=motor, pulse=1200, duration_ms=700, confirm="PROPS_REMOVED"))\n    await asyncio.sleep(0.9)\nawait drone.motor_stop()\n`;
const PY_ESC_CAL_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("ESC CALIBRATION • REMOVE ALL PROPELLERS and disconnect/reconnect power only as required by your ESC procedure.")\nprint(await drone.esc_calibrate(confirm="PROPS_REMOVED"))\nfor _ in range(8):\n    print(await drone.bench_status())\n    await asyncio.sleep(1)\n`;
const PY_GYRO_CAL_EXAMPLE=`from zebjus import Drone\n\ndrone = Drone()\nprint("Keep the frame LEVEL and COMPLETELY STILL.")\nprint(await drone.calibrate_gyro())\n`;
const PY_CAL_READ_EXAMPLE=`from zebjus import Drone\nimport json\n\ndrone = Drone()\nr = await drone.get_calibration()\nprint(json.dumps(r["calibration"], indent=2))\n`;
const PY_ACCEL_OFFSET_EXAMPLE=`from zebjus import Drone\nimport json\n\ndrone = Drone()\n# Additive accelerometer corrections in g. Edit these values, then Run.\nACC_X_OFFSET = -0.10\nACC_Y_OFFSET = 0.03\nACC_Z_OFFSET = 0.12\nROLL_TRIM_DEG = 0.0\nPITCH_TRIM_DEG = 0.0\nr = await drone.set_accel_offsets(ACC_X_OFFSET, ACC_Y_OFFSET, ACC_Z_OFFSET, ROLL_TRIM_DEG, PITCH_TRIM_DEG)\nprint(json.dumps(r["calibration"], indent=2))\n`;
const PY_LEVEL_CAL_EXAMPLE=`from zebjus import Drone\nimport json\n\ndrone = Drone()\nprint("Place the drone LEVEL and COMPLETELY STILL. Keep propellers removed.")\nr = await drone.level_calibrate(samples=400)\nprint(json.dumps(r["calibration"], indent=2))\n`;
const PY_PID_DEFAULTS_EXAMPLE=`from zebjus import Drone\n\ndrone = Drone()\nprint(await drone.restore_pid_defaults())\n`;
const PY_STATUS_EXAMPLE=`from zebjus import Drone\nimport json\n\ndrone = Drone()\nprint(json.dumps(await drone.status(), indent=2))\n`;
const PY_CUSTOM_COMBINED_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\n# Customize this combined project: PID + mode + transmitter + attitude monitor.\nawait drone.set_rate_pid(roll=(1.4,18,0.035), pitch=(1.4,18,0.035), yaw=(3,20,0))\nawait drone.rc(throttle=1000, arm=False, mode="rate")\nprint("Ready. This same file works with Simulator or Real Kit target.")\nfor _ in range(40):\n    a = await drone.attitude()\n    print(f"R={a['roll']:+.2f} P={a['pitch']:+.2f} Y={a['yaw']:+.2f}")\n    await asyncio.sleep(0.10)\n`;
const PY_LEVEL_CHECK_EXAMPLE=`from zebjus import Drone\nimport asyncio, math\n\ndrone = Drone()\nprint("Level checker • keep the frame still")\nwhile not drone.stop_requested():\n    a = await drone.attitude()\n    tilt = math.sqrt(a["roll"]**2 + a["pitch"]**2)\n    print(f"Roll={a['roll']:+.2f}° Pitch={a['pitch']:+.2f}°  total tilt={tilt:.2f}°  {'LEVEL' if tilt < 2 else 'TILTED'}")\n    await asyncio.sleep(0.15)\n`;
const PY_RX_MONITOR_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("Receiver monitor • CH1 Roll CH2 Pitch CH3 Throttle CH4 Yaw CH5 Arm CH6 Mode")\nwhile not drone.stop_requested():\n    r = await drone.receiver()\n    ch = r["channels"]\n    mode = "RATE" if ch[5] >= 1500 else "ANGLE"\n    arm = "ARM" if ch[4] >= 1500 else "DISARM"\n    print(f"{r.get('source','--'):>7}  R={ch[0]} P={ch[1]} T={ch[2]} Y={ch[3]}  {arm}  {mode}")\n    await asyncio.sleep(0.12)\n`;
const PY_IMU_STILL_EXAMPLE=`from zebjus import Drone\nimport asyncio, math\n\ndrone = Drone()\nprint("IMU stillness / vibration checker")\nwhile not drone.stop_requested():\n    g = await drone.gyro()\n    mag = math.sqrt(g["x"]**2 + g["y"]**2 + g["z"]**2)\n    print(f"gyro magnitude={mag:6.2f} dps  X={g['x']:+.2f} Y={g['y']:+.2f} Z={g['z']:+.2f}")\n    await asyncio.sleep(0.08)\n`;
const PY_SENSOR_SNAPSHOT_EXAMPLE=`from zebjus import Drone\nimport json\n\ndrone = Drone()\nprint("=== FLIGHTCORE STATUS ===")\nprint(json.dumps(await drone.status(), indent=2))\nprint("\\n=== I2C ===")\nprint(json.dumps(await drone.i2c_scan(), indent=2))\nprint("\\n=== IMU ===")\nprint(json.dumps(await drone.imu(), indent=2))\nprint("\\n=== RECEIVER ===")\nprint(json.dumps(await drone.receiver(), indent=2))\n`;
const PY_FLIGHT_DIAG_EXAMPLE=`from zebjus import Drone\nimport json\n\ndrone = Drone()\nfor title, fn in (("STATUS",drone.status),("ATTITUDE",drone.attitude),("RECEIVER",drone.receiver),("PID",drone.pid_get)):\n    print("\\n===", title, "===")\n    try: print(json.dumps(await fn(), indent=2))\n    except Exception as e: print("ERROR:", e)\n`;
const PY_RC_CENTER_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("Sending safe centered DISARMED RC frames for 2 seconds")\nfor _ in range(50):\n    await drone.rc(roll=1500,pitch=1500,throttle=1000,yaw=1500,arm=False,mode="angle")\n    await asyncio.sleep(0.04)\nprint("RC center complete")\n`;
const PY_THROTTLE_RAMP_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("Simulator throttle ramp • no real props / flight")\nfor t in range(1000,1501,10):\n    await drone.rc(throttle=t,arm=True,mode="angle")\n    await asyncio.sleep(0.04)\nfor t in range(1500,999,-15):\n    await drone.rc(throttle=t,arm=True,mode="angle")\n    await asyncio.sleep(0.04)\nawait drone.rc(throttle=1000,arm=False,mode="angle")\n`;
const PY_BENCH_STATUS_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nfor _ in range(20):\n    print(await drone.bench_status())\n    await asyncio.sleep(0.25)\n`;
const PY_MOTOR_ONE_EXAMPLE=`from zebjus import Drone\n\ndrone = Drone()\nprint("REMOVE ALL PROPELLERS. Testing only M1 at a guarded low pulse.")\nprint(await drone.motor_test(motor=1,pulse=1150,duration_ms=600,confirm="PROPS_REMOVED"))\n`;
const PY_PID_STEP_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\n# Simulator PID step-response teaching project. Edit these values and Rerun.\nPRateRoll=1.40\nIRateRoll=18.0\nDRateRoll=0.035\nPRatePitch=1.40\nIRatePitch=18.0\nDRatePitch=0.035\nPRateYaw=3.0\nIRateYaw=20.0\nDRateYaw=0.0\nawait drone.set_rate_pid(roll=(PRateRoll,IRateRoll,DRateRoll),pitch=(PRatePitch,IRatePitch,DRatePitch),yaw=(PRateYaw,IRateYaw,DRateYaw))\nawait drone.rc(throttle=1350,arm=True,mode="rate")\nawait asyncio.sleep(0.5)\nawait drone.rc(roll=1650,pitch=1500,throttle=1350,arm=True,mode="rate")\nawait asyncio.sleep(0.8)\nawait drone.rc(roll=1500,pitch=1500,throttle=1350,arm=True,mode="rate")\nawait asyncio.sleep(1.0)\nawait drone.rc(throttle=1000,arm=False,mode="rate")\nprint("Edit P/I/D and Rerun to compare the Tripod response.")\n`;
const PY_PLOT_IMU_EXAMPLE=`from zebjus import Drone\nimport asyncio\nimport matplotlib\nmatplotlib.use("Agg", force=True)  # Worker has no browser document\nimport matplotlib.pyplot as plt\n\ndrone = Drone()\nxs, ys, zs = [], [], []\nfor _ in range(40):\n    sample = (await drone.imu())["accel"]\n    xs.append(sample["x"]); ys.append(sample["y"]); zs.append(sample["z"])\n    await asyncio.sleep(0.1)\nfig, ax = plt.subplots(figsize=(7, 3))\nax.plot(xs, label="X"); ax.plot(ys, label="Y"); ax.plot(zs, label="Z")\nax.set(title="FlightCore acceleration", xlabel="Sample", ylabel="g")\nax.legend(); ax.grid(True)\ndrone.show_plot(fig)  # opens a movable, resizable output window\nplt.close(fig)\n`;
const PY_CAMERA_OPENCV_EXAMPLE=`from zebjus import Drone\nimport asyncio, cv2, numpy as np\n\ndrone = Drone()\nprint("Allow camera permission. OpenCV output appears in a movable window.")\nfor frame_no in range(100):\n    jpeg = await drone.camera_frame()\n    frame = cv2.imdecode(np.frombuffer(jpeg, dtype=np.uint8), cv2.IMREAD_COLOR)\n    if frame is None: raise RuntimeError("Camera frame could not be decoded")\n    edges = cv2.Canny(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY), 80, 160)\n    cv2.imshow("OpenCV edges", cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR))\n    if frame_no % 15 == 0: print("OpenCV frame", frame_no, "shape", frame.shape)\n    await drone.wait_key(60)\n`;
const PY_HAND_LANDMARKS_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("Camera and MediaPipe Hands start automatically. Allow browser permission.")\nfor _ in range(60):\n    data = await drone.hands()\n    for handedness, hand in zip(data["handedness"], data["landmarks"]):\n        tip = hand[8]  # normalized index fingertip x/y\n        print(f"{handedness}: fingertip x={tip['x']:.2f}, y={tip['y']:.2f}")\n    await asyncio.sleep(0.15)\n`;
const PY_CV_TELEMETRY_EXAMPLE=`from zebjus import Drone\nimport asyncio, cv2, numpy as np\n\ndrone = Drone()\nprint("Read-only overlay from the verified real kit. Allow camera permission.")\nfor frame_no in range(80):\n    jpeg = await drone.camera_frame()\n    frame = cv2.imdecode(np.frombuffer(jpeg, dtype=np.uint8), cv2.IMREAD_COLOR)\n    attitude = await drone.attitude()\n    label = f"R {attitude['roll']:+.1f}  P {attitude['pitch']:+.1f}  Y {attitude['yaw']:+.1f}"\n    cv2.putText(frame, label, (10, 28), cv2.FONT_HERSHEY_SIMPLEX, .55, (55, 235, 150), 2)\n    cv2.imshow("ZEBJUS camera + attitude", frame)\n    if frame_no % 15 == 0: print(label)\n    await drone.wait_key(80)\n`;
const PY_RC_RATE_EXAMPLE=`from zebjus import Drone\nimport asyncio\n\ndrone = Drone()\nprint("Measured FC rates: PPM frame input, Web/AP RC input, and 250 Hz loop separately.")\nwhile True:\n    status = await drone.status()\n    print(f"Source={status.get('rcSource','NONE')}  PPM={status.get('ppmFrameHz','--')} Hz  Web/AP={status.get('webRcFrameHz','--')} Hz  FC loop={status.get('flightLoopHz','--')} Hz  overruns={status.get('loopOverruns','--')}")\n    await asyncio.sleep(1.0)\n`;
const PY_PINMAP_EXAMPLE=`from zebjus import Drone

drone=Drone()
for m in (await drone.pinmap_get())["expansion"]["motors"]:
    print("M", m["motor"], "->", m["connector"], "GPIO", m["gpio"])
# With ALL props removed, set e.g. M1..M4 once and restart:
# await drone.motor_map_set(("D1","D2","D3","D0"))
`;
const PY_PPM_CONFIG_EXAMPLE=`from zebjus import Drone

drone=Drone()
print((await drone.receiver())["channels"][:6])
# Reverse only yaw. Select FALLING for an inverted PPM electrical signal.
print(await drone.ppm_config(edge="RISING",reverse=(False,False,False,True)))
print((await drone.receiver())["channels"][:6])
`;
const PY_MULTI_I2C_EXAMPLE=`from zebjus import Drone

drone=Drone()
scan=await drone.i2c_scan()
print("ACK devices:",scan["count"])
for dev in scan["devices"]:
    address=dev["address"]
    print(dev["addressHex"],dev.get("hint","device"))
    # Supply a register supported by that device's own datasheet.
    # Example: print(await drone.i2c_read(address,0x00,1))
`;
const PY_SERVO_EXAMPLE=`from zebjus import Drone
import asyncio

drone=Drone()
print(await drone.servo_config(17))  # A2 D7; external servo power and common ground
for pulse in (1500,1300,1500,1700,1500):
    print(await drone.servo_write(pulse))
    await asyncio.sleep(0.35)
`;
const PY_GPS_EXAMPLE=`from zebjus import Drone
import asyncio

drone=Drone()
print(await drone.gps_config(19))  # GPS TX to A2 D8 RX, 9600 baud
for _ in range(15):
    print(await drone.gps_read())
    await asyncio.sleep(1)
`;
const PY_MATRIX_EXAMPLE=`from zebjus import Drone

drone=Drone()
print(await drone.matrix_config(0x70))  # HT16K33 8x8 at 0x70
heart=[0,36,126,126,60,24,0,0]
print(await drone.matrix_write(heart))
print(await drone.matrix_read())
`;
const PY_GPIO_EXAMPLE=`from zebjus import Drone
import asyncio

drone=Drone()
for value in (1,0,1,0):
    print(await drone.gpio_write(18,value))  # A2 D10, 3.3V logic; use external driver for loads
    await asyncio.sleep(0.4)
print(await drone.gpio_read(18))
`;
const PY_EXAMPLES=[
 {id:'pinmap',title:'Motor Pin Map',desc:'Real M1–M4 connector and GPIO routing',code:PY_PINMAP_EXAMPLE,target:'real'},
 {id:'ppm-flip',title:'PPM Polarity + Flip',desc:'Edge and roll/pitch/throttle/yaw reversal',code:PY_PPM_CONFIG_EXAMPLE,target:'real'},
 {id:'multi-i2c',title:'Multiple I²C Devices',desc:'Scan bus and inspect device registers',code:PY_MULTI_I2C_EXAMPLE,target:'real'},
 {id:'servo',title:'Servo PWM',desc:'A2 spare pin at 50 Hz',code:PY_SERVO_EXAMPLE,target:'real'},
 {id:'gps',title:'GPS NMEA',desc:'A2 RX-only 9600 baud GPS sentences',code:PY_GPS_EXAMPLE,target:'real'},
 {id:'matrix',title:'8×8 LED Matrix',desc:'HT16K33 display at 0x70–0x77',code:PY_MATRIX_EXAMPLE,target:'real'},
 {id:'gpio',title:'Spare GPIO',desc:'A2 D7–D10 logic output/read',code:PY_GPIO_EXAMPLE,target:'real'},
 {id:'i2c',title:'I²C Scanner',desc:'Scan the real FlightCore I²C bus',code:PY_I2C_EXAMPLE,target:'real'},
 {id:'imu',title:'IMU Live',desc:'Accel + gyro live stream and graph',code:PY_IMU_EXAMPLE,target:'real'},
 {id:'gyro',title:'Gyroscope Read',desc:'Roll / pitch / yaw rates',code:PY_GYRO_EXAMPLE,target:'real'},
 {id:'accel',title:'Accelerometer Read',desc:'X/Y/Z acceleration in g',code:PY_ACCEL_EXAMPLE,target:'real'},
 {id:'angle',title:'Angle / Attitude Read',desc:'Real MPU6050 filtered roll/pitch + gyro-integrated yaw',code:PY_ANGLE_EXAMPLE,target:'real'},
 {id:'ppm',title:'PPM / Receiver Read',desc:'CH1-CH6 + active RC source',code:PY_PPM_EXAMPLE,target:'real'},
 {id:'status',title:'FlightCore Status',desc:'Board, firmware and capabilities',code:PY_STATUS_EXAMPLE,target:'real'},
 {id:'pid-read',title:'Read PID from FC',desc:'Read persistent Rate + Angle tuning',code:PY_PID_READ_EXAMPLE,target:'real'},
 {id:'rate-pid',title:'RATE PID Tuning',desc:'Tune RATE-mode PID; simulator mirrors code values',code:PY_RATE_PID_EXAMPLE,target:'sim'},
 {id:'angle-pid',title:'ANGLE PID Tuning',desc:'Outer Angle + inner Rate PID',code:PY_ANGLE_PID_EXAMPLE,target:'sim'},
 {id:'pid-defaults',title:'Restore PID Defaults',desc:'Restore the proven default PID profile',code:PY_PID_DEFAULTS_EXAMPLE,target:'sim'},
 {id:'keyboard-rc',title:'Keyboard Transmitter',desc:'Editable Python keys • opens live Joystick',code:PY_KEYBOARD_RC_EXAMPLE,target:'real'},
 {id:'rc-script',title:'Scripted RC Flight Demo',desc:'Programmatic throttle / roll demonstration',code:PY_RC_SCRIPT_EXAMPLE,target:'sim'},
 {id:'gyro-cal',title:'Gyro Calibration',desc:'2000-sample real MPU6050 gyro calibration',code:PY_GYRO_CAL_EXAMPLE,target:'real'},
 {id:'cal-read',title:'Read Level Calibration',desc:'Read accel offsets, gyro bias and level trim',code:PY_CAL_READ_EXAMPLE,target:'real'},
 {id:'acc-offsets',title:'Set Accel Offsets',desc:'Edit X/Y/Z g offsets + Roll/Pitch trim and save',code:PY_ACCEL_OFFSET_EXAMPLE,target:'real'},
 {id:'level-cal',title:'Auto Level Calibration',desc:'Capture level accel offsets + gyro bias',code:PY_LEVEL_CAL_EXAMPLE,target:'real'},
 {id:'motor-test',title:'Motor Sequence Test',desc:'Bench M1-M4 test • props removed',code:PY_MOTOR_TEST_EXAMPLE,target:'real'},
 {id:'esc-cal',title:'ESC Calibration',desc:'3 s high + 3 s low bench calibration',code:PY_ESC_CAL_EXAMPLE,target:'real'},
 {id:'level-check',title:'Level Checker',desc:'Real MPU6050 roll/pitch + total tilt check',code:PY_LEVEL_CHECK_EXAMPLE,target:'real'},
 {id:'rx-monitor',title:'Receiver Channel Monitor',desc:'Readable CH1-CH6 + source/mode/arm',code:PY_RX_MONITOR_EXAMPLE,target:'real'},
 {id:'imu-still',title:'IMU Stillness / Vibration',desc:'Gyro magnitude for vibration checks',code:PY_IMU_STILL_EXAMPLE,target:'real'},
 {id:'sensor-snapshot',title:'Sensor Snapshot',desc:'Status + I²C + IMU + receiver one-shot',code:PY_SENSOR_SNAPSHOT_EXAMPLE,target:'real'},
 {id:'flight-diag',title:'Flight Diagnostics',desc:'Status + attitude + receiver + PID report',code:PY_FLIGHT_DIAG_EXAMPLE,target:'real'},
 {id:'rc-center',title:'Safe RC Center',desc:'Send centered, disarmed RC frames',code:PY_RC_CENTER_EXAMPLE,target:'sim'},
 {id:'throttle-ramp',title:'Simulator Throttle Ramp',desc:'Programmed throttle ramp + stick reflection',code:PY_THROTTLE_RAMP_EXAMPLE,target:'sim'},
 {id:'bench-status',title:'Bench Status Monitor',desc:'Watch motor / ESC bench state',code:PY_BENCH_STATUS_EXAMPLE,target:'real'},
 {id:'motor-one',title:'Single Motor Test',desc:'Guarded low-pulse M1 bench test',code:PY_MOTOR_ONE_EXAMPLE,target:'real'},
 {id:'pid-step',title:'RATE PID Step Response',desc:'Edit PID and compare Tripod response',code:PY_PID_STEP_EXAMPLE,target:'sim'},
 {id:'rc-rate',title:'RC Input + FC Loop Hz',desc:'Read measured PPM/Web receive and 250 Hz loop rates',code:PY_RC_RATE_EXAMPLE,target:'real'},
 {id:'plot-imu',title:'Plot IMU / Matplotlib',desc:'Collect 40 real IMU samples and show a graph',code:PY_PLOT_IMU_EXAMPLE,target:'real'},
 {id:'camera-opencv',title:'Camera / OpenCV Edges',desc:'Browser camera frames processed with cv2',code:PY_CAMERA_OPENCV_EXAMPLE,target:'sim'},
 {id:'hand-landmarks',title:'MediaPipe Hand Landmarks',desc:'Read browser Hand Landmarker results in Python',code:PY_HAND_LANDMARKS_EXAMPLE,target:'sim'},
 {id:'cv-telemetry',title:'Camera + Kit Telemetry',desc:'Overlay real kit attitude on a camera frame',code:PY_CV_TELEMETRY_EXAMPLE,target:'real'},
 {id:'combined',title:'Combined Custom Project',desc:'PID + RC + attitude in one editable project',code:PY_CUSTOM_COMBINED_EXAMPLE,target:'sim'}
];
const ZEBJUS_PY_PRELUDE=`import sys, types, json, asyncio\nfrom js import zebjusBridge as _zebjus_bridge\n\nif "/home/pyproject" not in sys.path:\n    sys.path.insert(0, "/home/pyproject")\n\n_zebjus_module = types.ModuleType("zebjus")\n\nclass Drone:\n    def target(self):\n        return str(_zebjus_bridge.target())\n\n    def stop_requested(self):\n        return bool(_zebjus_bridge.stopRequested())\n\n    async def _command(self, command, **data):\n        raw = await _zebjus_bridge.command(command, json.dumps(data))\n        return json.loads(str(raw))\n\n    async def status(self):\n        raw = await _zebjus_bridge.status()\n        return json.loads(str(raw))\n\n    async def i2c_scan(self):\n        raw = await _zebjus_bridge.i2cScan()\n        return json.loads(str(raw))\n\n    async def imu(self):\n        last_error = None\n        for _ in range(3):\n            try:\n                raw = await _zebjus_bridge.imuRead()\n                return json.loads(str(raw))\n            except Exception as exc:\n                last_error = exc\n                await asyncio.sleep(0.06)\n        raise RuntimeError("Temporary IMU read failure: " + str(last_error))\n\n    async def gyro(self): return (await self.imu())["gyro"]\n    async def accel(self): return (await self.imu())["accel"]\n    async def attitude(self): return await self._command("attitude_read")\n    async def receiver(self): return await self._command("receiver_read")\n    async def ppm(self): return await self.receiver()\n    async def pid_get(self): return await self._command("pid_get")\n    async def restore_pid_defaults(self): return await self._command("pid_defaults")\n    async def set_rate_pid(self, roll=None, pitch=None, yaw=None):\n        d = {}\n        for name, vals in (("rateRoll",roll),("ratePitch",pitch),("rateYaw",yaw)):\n            if vals is not None:\n                d[name+"P"],d[name+"I"],d[name+"D"] = vals\n        return await self._command("pid_set", **d)\n    async def set_angle_pid(self, outer_roll=None, outer_pitch=None, inner_roll=None, inner_pitch=None, inner_yaw=None):\n        d = {}\n        for name, vals in (("angleRoll",outer_roll),("anglePitch",outer_pitch),("angleRateRoll",inner_roll),("angleRatePitch",inner_pitch),("angleRateYaw",inner_yaw)):\n            if vals is not None:\n                d[name+"P"],d[name+"I"],d[name+"D"] = vals\n        return await self._command("pid_set", **d)\n    async def rc(self, roll=1500, pitch=1500, throttle=1000, yaw=1500, arm=False, mode="angle", extra=None):\n        ch=[int(roll),int(pitch),int(throttle),int(yaw),2000 if arm else 1000,1500 if str(mode).lower()=="rate" else 1000,1000,1000,1500,1000]\n        if extra:\n            for i,v in enumerate(extra[:4],start=6): ch[i]=int(v)\n        return await self._command("rc_frame", channels=ch)\n    async def keys(self):\n        raw = await _zebjus_bridge.keyboardState()\n        return json.loads(str(raw))\n    async def motor_test(self, motor=1, pulse=1200, duration_ms=700, confirm=""):\n        return await self._command("motor_test", motor=motor, pulse=pulse, durationMs=duration_ms, confirm=confirm)\n    async def motor_stop(self): return await self._command("motor_stop")\n    async def motor_order_test(self, confirm=""): return await self._command("motor_order_test", confirm=confirm)\n    async def esc_calibrate(self, confirm=""): return await self._command("esc_calibrate", confirm=confirm)\n    async def bench_status(self): return await self._command("bench_status")\n    async def get_calibration(self): return await self._command("calibration_get")\n    async def set_accel_offsets(self, x, y, z, trim_roll=0.0, trim_pitch=0.0):\n        return await self._command("calibration_set", accelOffsetX=x, accelOffsetY=y, accelOffsetZ=z, levelTrimRoll=trim_roll, levelTrimPitch=trim_pitch)\n    async def level_calibrate(self, samples=400): return await self._command("level_calibrate", samples=int(samples))\n    async def restore_calibration_defaults(self): return await self._command("calibration_defaults")\n    async def calibrate_gyro(self): return await self._command("calibrate_gyro")\n    async def pinmap_get(self): return await self._command("pinmap_get")\n    async def motor_map_get(self): return await self.pinmap_get()\n    async def motor_map_set(self, slots=("D1", "D2", "D3", "D0")):\n        if sorted(str(x).upper() for x in slots) != ["D0", "D1", "D2", "D3"]: raise ValueError("Assign D0-D3 exactly once to M1-M4")\n        return await self._command("motor_map_set", **{"m"+str(i+1)+"slot":int(str(v)[1]) for i,v in enumerate(slots)})\n    async def ppm_config(self, edge="RISING", reverse=(False,False,False,False)):\n        if len(reverse)!=4: raise ValueError("Specify roll, pitch, throttle, yaw reversals")\n        return await self._command("ppm_config", edge=str(edge).upper(), **{"reverse"+str(i):int(bool(v)) for i,v in enumerate(reverse)})\n    async def i2c_read(self, address, reg, length=1): return await self._command("i2c_read", address=int(address), reg=int(reg), length=int(length))\n    async def i2c_write(self, address, reg, values): return await self._command("i2c_write", address=int(address), reg=int(reg), bytes=",".join(str(int(x)) for x in values))\n    async def servo_config(self, pin): return await self._command("servo_config", pin=int(pin))\n    async def servo_write(self, pulse_us=1500): return await self._command("servo_write", pulseUs=int(pulse_us))\n    async def gps_config(self, pin): return await self._command("gps_config", pin=int(pin))\n    async def gps_read(self): return await self._command("gps_read")\n    async def matrix_config(self, address=0x70): return await self._command("matrix_config", address=int(address))\n    async def matrix_write(self, rows):\n        if len(rows)!=8: raise ValueError("Specify eight rows of 0-255")\n        return await self._command("matrix_write", rows=",".join(str(int(x)) for x in rows))\n    async def matrix_read(self): return await self._command("matrix_read")\n    async def gpio_read(self, pin): return await self._command("gpio_read", pin=int(pin))\n    async def gpio_write(self, pin, value): return await self._command("gpio_write", pin=int(pin), value=int(bool(value)))\n    async def gpio_release(self, pin): return await self._command("gpio_release", pin=int(pin))\n\n\n_zebjus_module.Drone = Drone\nsys.modules["zebjus"] = _zebjus_module\n`;
...
function loadPythonExample(id){const ex=PY_EXAMPLES.find(x=>x.id===id);if(!ex)return;pythonProjectHistoryPush();pythonFiles[pythonActiveFile]=getPythonCode();setPythonCode(ex.code);pythonFiles[pythonActiveFile]=ex.code;if(ex.target&&$('#pythonTarget'))$('#pythonTarget').value=ex.target;savePythonProject();clearPythonMarkers();const term=$('#pythonTerminal');if(term){const targetName=ex.target==='real'?'Real ZEBJUS kit':'Simulator';term.textContent=`[READY] ${ex.title} loaded.\nTarget: ${targetName}\nPress Run.\n`;term.scrollTop=term.scrollHeight}$('#pyStatus').textContent=`${ex.title} ready`;ensureMonaco()}
function exportPythonProject(){pythonFiles[pythonActiveFile]=getPythonCode();const payload={schema:1,product:'ZEBJUS Python Lab',version:'18.3.46',exportedAt:new Date().toISOString(),active:pythonActiveFile,files:pythonFiles};downloadBlob('ZEBJUS_Python_Project.json',JSON.stringify(payload,null,2),'application/json')}
function importPythonProject(file){if(!file)return;const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(String(r.result||''));if(!d?.files||typeof d.files!=='object')throw new Error('Missing files map');const clean={};for(const [name,code] of Object.entries(d.files)){if(!/^[A-Za-z0-9_.-]+\.py$/.test(name))continue;clean[name]=String(code??'')}if(!Object.keys(clean).length)throw new Error('No valid .py files');pythonProjectHistoryPush();pythonFiles=clean;pythonActiveFile=clean[d.active]!=null?d.active:Object.keys(clean)[0];setPythonCode(clean[pythonActiveFile]);renderPythonFiles();savePythonProject();clearPythonMarkers();$('#pyStatus').textContent='Python project imported'}catch(e){alert('Invalid ZEBJUS Python project: '+e.message)}};r.readAsText(file)}
function normalizeImuSample(d){if(!d)return null;const a=d.accel||{},g=d.gyro||{};const ax=Number(a.x??d.accX),ay=Number(a.y??d.accY),az=Number(a.z??d.accZ),gx=Number(g.x??d.gyroX),gy=Number(g.y??d.gyroY),gz=Number(g.z??d.gyroZ);if(![ax,ay,az,gx,gy,gz].every(Number.isFinite))return null;return{t:performance.now(),wall:new Date().toISOString(),sensor:d.sensor||d.imuModel||'IMU',ax,ay,az,gx,gy,gz}}
function recordImuSample(data){const v=normalizeImuSample(data);if(!v)return;imuPrevAt=imuLastAt;imuLastAt=v.t;imuToolSamples.push(v);if(imuToolSamples.length>180)imuToolSamples.shift();if(imuRecording)imuCsvRows.push(v);const sensor=$('#imuToolSensor'),rate=$('#imuToolRate'),age=$('#imuToolAge'),rec=$('#imuToolRecord');if(sensor)sensor.textContent=v.sensor;if(rate)rate.textContent=imuPrevAt?`${(1000/Math.max(1,imuLastAt-imuPrevAt)).toFixed(1)} Hz`:'-- Hz';if(age)age.textContent='0 ms';if(rec)rec.textContent=imuRecording?`${imuCsvRows.length} rows`:'OFF';drawImuTools()}
function drawMiniImu(canvasId,fields,scale,label){const c=$(canvasId);if(!c)return;const r=c.getBoundingClientRect(),dpr=Math.min(devicePixelRatio||1,2),w=Math.max(260,Math.round(r.width||520)),h=Math.max(90,Math.round(r.height||120));if(c.width!==w*dpr||c.height!==h*dpr){c.width=w*dpr;c.height=h*dpr}const x=c.getContext('2d');x.setTransform(dpr,0,0,dpr,0,0);x.clearRect(0,0,w,h);x.fillStyle='#06121b';x.fillRect(0,0,w,h);x.strokeStyle='#173447';x.lineWidth=1;for(let i=1;i<4;i++){const y=i*h/4;x.beginPath();x.moveTo(0,y);x.lineTo(w,y);x.stroke()}const cols=['#62e7ba','#55b7ff','#f6c85f'];fields.forEach((f,fi)=>{x.strokeStyle=cols[fi];x.lineWidth=1.6;x.beginPath();imuToolSamples.forEach((v,i)=>{let value=v[f]-Number(imuCal[f]||0);const px=i/Math.max(1,imuToolSamples.length-1)*w,py=h/2-clamp(value/scale,-1,1)*h*.42;i?x.lineTo(px,py):x.moveTo(px,py)});x.stroke()});x.fillStyle='#91aabd';x.font='10px system-ui';x.fillText(label,8,14);x.fillText('X',w-62,14);x.fillStyle=cols[1];x.fillText('Y',w-42,14);x.fillStyle=cols[2];x.fillText('Z',w-22,14)}
function drawImuTools(){drawMiniImu('#imuAccelChart',['ax','ay','az'],2,'ACC ±2 g');drawMiniImu('#imuGyroChart',['gx','gy','gz'],250,'GYRO ±250 dps')}
function updateImuToolAge(){const e=$('#imuToolAge');if(e)e.textContent=imuLastAt?`${Math.round(performance.now()-imuLastAt)} ms`:'-- ms'}
function calibrateImuTeaching(){if(imuToolSamples.length<20){notify('Collect at least 20 IMU samples while the sensor is level and still.','bad');return}const a=imuToolSamples.slice(-60),avg=k=>a.reduce((n,v)=>n+v[k],0)/a.length;imuCal={ax:avg('ax'),ay:avg('ay'),az:avg('az')-1,gx:avg('gx'),gy:avg('gy'),gz:avg('gz')};try{localStorage.setItem('zebjus-imu-teaching-cal',JSON.stringify(imuCal))}catch{};$('#imuToolNote').textContent=`Browser teaching offset saved • gyro ${imuCal.gx.toFixed(2)}, ${imuCal.gy.toFixed(2)}, ${imuCal.gz.toFixed(2)} dps. Persistent real-FC level calibration is available in the Calibration tab.`;drawImuTools();notify('Browser IMU level offset saved.','good')}
function toggleImuRecording(){imuRecording=!imuRecording;if(imuRecording)imuCsvRows=[];const b=$('#imuRecordBtn'),r=$('#imuToolRecord'),d=$('#imuCsvBtn');if(b)b.textContent=imuRecording?'Stop recording':'Record CSV';if(r)r.textContent=imuRecording?'0 rows':'OFF';if(d)d.disabled=imuRecording||!imuCsvRows.length}
function downloadImuCsv(){if(!imuCsvRows.length)return;const head='time,sensor,ax_g,ay_g,az_g,gx_dps,gy_dps,gz_dps\n',body=imuCsvRows.map(v=>[v.wall,v.sensor,v.ax,v.ay,v.az,v.gx,v.gy,v.gz].join(',')).join('\n');downloadBlob(`ZEBJUS_IMU_${Date.now()}.csv`,head+body,'text/csv')}
...
function monacoCompletions(monaco,model,position){
 ...
 const items=[
  ['print','print()',K.Function,'Print to the WebApp terminal'],['input','input()',K.Function,'Read one line of input'],['if','if condition:\n    pass',K.Snippet,'Conditional block'],['for','for item in range(5):\n    print(item)',K.Snippet,'For loop'],['while','while condition:\n    pass',K.Snippet,'While loop'],['def','def function_name():\n    pass',K.Snippet,'Define a function'],['class','class ClassName:\n    def __init__(self):\n        pass',K.Snippet,'Define a class'],['try','try:\n    pass\nexcept Exception as e:\n    print(e)',K.Snippet,'Exception handling'],
  ['from zebjus import Drone','from zebjus import Drone',K.Module,'Import the ZEBJUS Drone Python API'],['Drone','Drone()',K.Class,'Create a drone API object'],['drone','drone = Drone()',K.Variable,'Create the default drone object'],['i2c_scan','await drone.i2c_scan()',K.Method,'Return real I²C scan data'],['imu','await drone.imu()',K.Method,'Return auto-detected real IMU accel + gyro data'],['gyro','await drone.gyro()',K.Method,'Return real gyroscope data'],['accel','await drone.accel()',K.Method,'Return real accelerometer data'],['status','await drone.status()',K.Method,'Read selected ZEBJUS kit status'],['stop_requested','drone.stop_requested()',K.Method,'True after the Stop button is pressed'],['asyncio','import asyncio',K.Module,'Import Python asyncio'],['json','import json',K.Module,'Import Python JSON support']
 ];
 const code=model.getValue(),seen=new Set(items.map(x=>x[0])),user=[];
 const add=(name,kind,detail)=>{if(!name||seen.has(name)||/^_/.test(name))return;seen.add(name);user.push([name,name,kind,detail])};
 for(const m of code.matchAll(/^\s*([A-Za-z_]\w*)\s*=\s*/gm))add(m[1],K.Variable,'Variable from your code');
 for(const m of code.matchAll(/^\s*for\s+([A-Za-z_]\w*)\s+in\s+/gm))add(m[1],K.Variable,'Loop variable from your code');
 for(const m of code.matchAll(/^\s*(?:async\s+)?def\s+([A-Za-z_]\w*)\s*\(/gm))add(m[1],K.Function,'Function from your code');
 for(const m of code.matchAll(/^\s*class\s+([A-Za-z_]\w*)/gm))add(m[1],K.Class,'Class from your code');
 for(const m of code.matchAll(/^\s*(?:from\s+\S+\s+import|import)\s+([A-Za-z_]\w*)/gm))add(m[1],K.Module,'Imported name from your code');
 const userNames=new Set(user.map(x=>x[0]));
 return{suggestions:[...user,...items].map(([label,insertText,kind,documentation])=>({label,kind,insertText,documentation,range,sortText:(userNames.has(label)?'0':'1')+label,insertTextRules:kind===K.Snippet?monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet:undefined}))}
}
function ensureMonaco(){
 ...
 monacoPromise=new Promise((resolve,reject)=>{
  const start=()=>{
   ...
   window.MonacoEnvironment={getWorkerUrl:()=>`data:text/javascript;charset=utf-8,${encodeURIComponent(`self.MonacoEnvironment={baseUrl:'${MONACO_BASE}'};importScripts('${MONACO_BASE}vs/base/worker/workerMain.js');`)}`};amd.config({paths:{vs:MONACO_BASE+'vs'}});
   amd(['vs/editor/editor.main'],()=>{try{const m=window.monaco;m.editor.defineTheme('zebjus-pycharm',{base:'vs-dark',inherit:true,rules:[{token:'comment',foreground:'7F8C98',fontStyle:'italic'},{token:'keyword',foreground:'CC78FA'},{token:'string',foreground:'A5C261'},{token:'number',foreground:'6897BB'},{token:'type.identifier',foreground:'FFC66D'},{token:'identifier',foreground:'D8E2EA'}],colors:{'editor.background':'#06101A','editor.foreground':'#D8E2EA','editorLineNumber.foreground':'#52697A','editorLineNumber.activeForeground':'#A8C7D9','editorCursor.foreground':'#69E6B5','editor.selectionBackground':'#214B5B88','editorSuggestWidget.background':'#0B1824','editorSuggestWidget.border':'#29475B'}});pythonMonaco=m.editor.create(host,{value:fallback.value||pythonFiles[pythonActiveFile]||PY_DEFAULT,language:'python',theme:'zebjus-pycharm',automaticLayout:true,fontSize:13,lineHeight:21,fontFamily:'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace',minimap:{enabled:false},scrollBeyondLastLine:false,tabSize:4,insertSpaces:true,wordWrap:'on',wrappingIndent:'indent',scrollbar:{vertical:'visible',horizontal:'visible',verticalScrollbarSize:13,horizontalScrollbarSize:13},quickSuggestionsDelay:80,quickSuggestions:{other:true,comments:false,strings:true},wordBasedSuggestions:'currentDocument',suggestOnTriggerCharacters:true,acceptSuggestionOnEnter:'on',tabCompletion:'on',autoClosingBrackets:'always',autoClosingQuotes:'always',autoIndent:'full',formatOnType:true,parameterHints:{enabled:true},bracketPairColorization:{enabled:true},guides:{bracketPairs:true,indentation:true},padding:{top:12,bottom:12},glyphMargin:true});m.languages.registerCompletionItemProvider('python',{triggerCharacters:['.',' ','_'],provideCompletionItems:(model,position)=>monacoCompletions(m,model,position)});pythonMonaco.addCommand(m.KeyMod.CtrlCmd|m.KeyCode.Enter,()=>$('#runPythonBtn')?.click());pythonMonaco.onDidChangeModelContent(()=>{pythonFiles[pythonActiveFile]=pythonMonaco.getValue();savePythonProject()});fallback.hidden=true;host.hidden=false;$('#pyStatus').textContent='PyCharm-style Python editor ready';resolve(pythonMonaco)}catch(e){reject(e)}},reject)
  };
  if(typeof window.require==='function'&&window.monaco){start();return}const sc=document.createElement('script');sc.src=MONACO_BASE+'vs/loader.js';sc.async=true;sc.onload=start;sc.onerror=()=>reject(new Error('Smart editor CDN unavailable'));document.head.appendChild(sc)
 }).catch(e=>{console.warn('[ZEBJUS] Monaco fallback:',e);host.hidden=true;fallback.hidden=false;$('#pyStatus').textContent='Basic editor fallback active';return null});return monacoPromise
}
...
function markPythonError(error){const text=String(error?.message||error||''),lines=[...text.matchAll(/File ["']([^"']+)["'], line (\d+)/g)],match=lines.reverse().find(m=>m[1].split('/').pop()===pythonActiveFile)||text.match(/line (\d+)/),line=Math.max(1,Number(match?.[2]||match?.[1]||0));if(!match)return;const message=(text.trim().split('\n').filter(Boolean).pop()||'Python error');if(window.monaco&&pythonMonaco){const model=pythonMonaco.getModel(),n=Math.min(line,model.getLineCount());window.monaco.editor.setModelMarkers(model,'zebjus-python',[{startLineNumber:n,endLineNumber:n,startColumn:1,endColumn:Math.max(2,model.getLineMaxColumn(n)),message,severity:window.monaco.MarkerSeverity.Error}]);pythonMonaco.revealLineInCenter(n);pythonMonaco.setPosition({lineNumber:n,column:1})}else{const editor=$('#pythonEditor'),rows=editor?.value.split('\n')||[];if(editor&&rows.length){const start=rows.slice(0,line-1).join('\n').length+(line>1?1:0);editor.focus();editor.setSelectionRange(start,start+(rows[line-1]||'').length)}}$('#pyStatus').textContent=`Python error at ${pythonActiveFile}:${line}`}
...
function pythonTerminalWrite(text,kind='out'){const term=$('#pythonTerminal');if(!term)return;const value=String(text||'');if(!value)return;const span=document.createElement('span');span.className=kind==='error'?'python-terminal-error':kind==='warn'?'python-terminal-warn':'python-terminal-out';span.textContent=value;term.appendChild(span);if(pythonRcProject){const joyLog=$('#joyPythonLog');if(joyLog){joyLog.textContent=(joyLog.textContent+value).slice(-7000);joyLog.scrollTop=joyLog.scrollHeight}}if(term.textContent.length>64000){const keep=term.textContent.slice(-56000);term.textContent='… older terminal output trimmed …\n'+keep}term.scrollTop=term.scrollHeight}
async function copyPythonTerminal(){const term=$('#pythonTerminal'),btn=$('#copyTerminalBtn');if(!term)return;const text=term.textContent||'';if(!text.trim()){notify('Terminal is empty.','bad');return}let ok=false;try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(text);ok=true}}catch{}if(!ok){const ta=document.createElement('textarea');ta.value=text;ta.setAttribute('readonly','');ta.style.position='fixed';ta.style.opacity='0';ta.style.pointerEvents='none';document.body.appendChild(ta);ta.select();try{ok=document.execCommand('copy')}catch{}ta.remove()}if(ok){if(btn){const old=btn.textContent;btn.textContent='Copied ✓';btn.classList.add('copied');setTimeout(()=>{btn.textContent=old;btn.classList.remove('copied')},1400)}notify('Terminal output copied.','good')}else notify('Copy failed. Select the terminal text and use Ctrl/⌘+C.','bad')}
function pythonPhysicalSensorNeeded(code){return /\b(?:i2c_scan|i2c_read|i2c_write|pinmap_get|motor_map_get|motor_map_set|ppm_config|servo_config|servo_write|gps_config|gps_read|matrix_config|matrix_write|matrix_read|gpio_read|gpio_write|gpio_release|imu\s*\(|gyro\s*\(|accel\s*\()/.test(String(code||''))}
function pythonKitCommandNeeded(code){return /\b(?:pinmap_get|motor_map_get|motor_map_set|ppm_config|i2c_read|i2c_write|servo_config|servo_write|gps_config|gps_read|matrix_config|matrix_write|matrix_read|gpio_read|gpio_write|gpio_release|pid_get|set_rate_pid|set_angle_pid|restore_pid_defaults|get_calibration|set_accel_offsets|level_calibrate|restore_calibration_defaults|receiver\s*\(|ppm\s*\(|rc\s*\(|motor_test|motor_order_test|motor_stop|esc_calibrate|bench_status|calibrate_gyro|attitude\s*\()/.test(String(code||''))}
function pythonHardwarePreflight(code){
 const target=$('#pythonTarget')?.value||'sim',physical=pythonPhysicalSensorNeeded(code),kitCommand=pythonKitCommandNeeded(code);if(target==='sim'&&physical){const term=$('#pythonTerminal');if(term){term.textContent='';pythonTerminalWrite('[TARGET] Hardware I/O requires “Real ZEBJUS kit”.\n','error')}$('#pyStatus').textContent='Real hardware target required';return false}
 if(target==='real'&&(physical||kitCommand)&&!window.zebjusSchool?.isKitActive?.()){const term=$('#pythonTerminal');if(term){term.textContent='';pythonTerminalWrite('[HARDWARE] The selected ZEBJUS kit is not VERIFIED CONNECTED.\nOpen Kit Connect → connect the selected Device ID → return to Python Lab → Run. Mutating Python commands acquire control automatically when available.\n','error')}$('#pyStatus').textContent='Verified kit connection required';notify('Connect the selected ZEBJUS kit first.','bad');return false}
 return true
}
function pythonFriendlyError(error,stderr=''){
 const all=`${stderr}\n${error?.message||error||''}`;
 if(/Connect .*ZEBJUS kit first|kit is not connected/i.test(all))return 'ZEBJUS kit is not connected. Open Kit Connect, connect the kit, then Run again.';
 if(/Select Real ZEBJUS kit/i.test(all))return 'Select “Real ZEBJUS kit” as the Python target, then Run again.';
 if(/IMU.*not found|LSM6DS3.*not found|MPU6050.*not found/i.test(all))return 'Supported IMU was not found. Check VCC/GND/SDA/SCL and supported addresses: LSM6DS3 0x6A/0x6B or MPU6050 0x68/0x69.';
 if(/PID edit blocked while armed|PID restore blocked while armed/i.test(all))return 'PID tuning is blocked while the real drone is ARMED. Land, DISARM, then send the PID values again.';
 if(/Take Control|Could not acquire control|Another browser is controlling/i.test(all))return 'This command changes real hardware. Python tried to acquire control automatically, but control is unavailable. Check the selected Device ID and release any other controlling browser.';
 if(/requires confirm=PROPS_REMOVED/i.test(all))return 'Bench motor/ESC command blocked. REMOVE PROPELLERS, keep the frame secured, then run the example with confirm="PROPS_REMOVED".';
 if(/Another browser is controlling/i.test(all))return 'Another browser currently owns this kit control lock. Release that session or wait for its lock to expire.';
 if(/PPM|receiver.*NONE|no.*receiver/i.test(all))return 'No fresh receiver signal is available. Check PPM wiring/signal or use the Web/AP RC source.';
...
function pythonSetRunUi(running){pythonRunning=!!running;const run=$('#runPythonBtn'),rerun=$('#rerunPythonBtn'),stop=$('#stopPythonBtn'),target=$('#pythonTarget'),quick=$('#pythonQuickHardware');if(run)run.disabled=running;if(rerun)rerun.disabled=running||!pythonHasRun;if(stop){stop.disabled=!running;stop.classList.toggle('python-stop-live',running)}if(target)target.disabled=running;if(quick)quick.disabled=running}
let pythonCameraStream=null,pythonHandLandmarker=null,pythonHandTimer=0,pythonCameraCanvas=null,pythonCameraPump=0,pythonRcProject=false,pythonRealRcActive=false,pythonLastHardwareCommand=Promise.resolve();let pythonHandData={landmarks:[],handedness:[],timestampMs:0};
function pythonCameraNote(message){const n=$('#pythonCameraStatus');if(n)n.textContent=message}
async function startPythonCamera(){if(pythonCameraStream)return;const video=$('#pythonCameraVideo');if(!video||!navigator.mediaDevices?.getUserMedia)throw new Error('Camera requires HTTPS/localhost. If this Lab is embedded, allow camera in the parent iframe.');try{pythonCameraStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:{ideal:640},height:{ideal:480}},audio:false});video.srcObject=pythonCameraStream;await video.play();video.closest('details').open=true;pythonCameraNote('Camera ready • live frames available in Python.')}catch(e){pythonCameraStream?.getTracks().forEach(t=>t.stop());pythonCameraStream=null;throw new Error(`Camera unavailable (${e.name||e.message}). Allow camera permission, use HTTPS/localhost, and set iframe allow="camera" when embedded.`)}}
function stopPythonHands(){if(pythonHandTimer){clearInterval(pythonHandTimer);pythonHandTimer=0}pythonHandData={landmarks:[],handedness:[],timestampMs:0};$('#pythonCameraOverlay')?.getContext('2d')?.clearRect(0,0,$('#pythonCameraOverlay')?.width||0,$('#pythonCameraOverlay')?.height||0);const btn=$('#pythonHandsToggle');if(btn)btn.textContent='Start hands'}
function stopPythonCamera(){stopPythonHands();pythonCameraStream?.getTracks().forEach(t=>t.stop());pythonCameraStream=null;const video=$('#pythonCameraVideo');if(video)video.srcObject=null;pythonCameraNote('Camera stopped.');}
function pythonCameraFrame(){const video=$('#pythonCameraVideo');if(!pythonCameraStream||!video||video.readyState<2)throw new Error('Press Start camera in Python Lab first.');if(window.zebjusSchool?.getSelectedDevice?.()?.armed&&$('#pythonTarget')?.value==='real')throw new Error('Camera processing is paused while the real kit is armed.');pythonCameraCanvas=pythonCameraCanvas||document.createElement('canvas');const canvas=pythonCameraCanvas;canvas.width=320;canvas.height=240;canvas.getContext('2d',{willReadFrequently:true}).drawImage(video,0,0,320,240);return canvas.toDataURL('image/jpeg',.68).split(',')[1]}
function startPythonCameraPump(worker){clearInterval(pythonCameraPump);const push=()=>{if(worker!==pythonWorker||!pythonCameraStream||document.hidden)return;try{worker.postMessage({type:'camera-frame',jpeg:pythonCameraFrame()})}catch{}};push();pythonCameraPump=setInterval(push,90)}
async function togglePythonHands(){if(pythonHandTimer){stopPythonHands();pythonCameraNote('Hand tracking stopped.');return}try{await startPythonCamera();pythonCameraNote('Loading MediaPipe hand model…');const {FilesetResolver,HandLandmarker}=await import('https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.21/vision_bundle.mjs');const files=await FilesetResolver.forVisionTasks('https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.21/wasm');pythonHandLandmarker=pythonHandLandmarker||await HandLandmarker.createFromOptions(files,{baseOptions:{modelAssetPath:'https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task',delegate:'CPU'},runningMode:'VIDEO',numHands:2});const video=$('#pythonCameraVideo'),canvas=$('#pythonCameraOverlay');let lastVideoTime=-1;pythonHandTimer=setInterval(()=>{if(!pythonCameraStream||document.hidden||!video||video.readyState<2||video.currentTime===lastVideoTime)return;if($('#pythonTarget')?.value==='real'&&window.zebjusSchool?.getSelectedDevice?.()?.armed)return;lastVideoTime=video.currentTime;try{const result=pythonHandLandmarker.detectForVideo(video,performance.now());pythonHandData={landmarks:(result.landmarks||[]).map(hand=>hand.map(p=>({x:p.x,y:p.y,z:p.z}))),handedness:(result.handedness||[]).map(x=>x?.[0]?.categoryName||'Unknown'),timestampMs:Date.now()};canvas.width=video.videoWidth||640;canvas.height=video.videoHeight||480;const ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);ctx.fillStyle='#61efb9';for(const hand of pythonHandData.landmarks)for(const point of hand){ctx.beginPath();ctx.arc(point.x*canvas.width,point.y*canvas.height,3,0,Math.PI*2);ctx.fill()}pythonCameraNote(`${pythonHandData.landmarks.length} hand(s) • data available through drone.hands()`)}catch(e){stopPythonHands();pythonCameraNote('MediaPipe error: '+e.message)}},140);$('#pythonHandsToggle').textContent='Stop hands'}catch(e){pythonCameraNote('MediaPipe could not start: '+e.message)}}
function openPythonOutputWindow(src,title='Python output',mime='image/png'){
 const win=$('#pythonOutputWindow'),img=$('#pythonOutputImage'),link=$('#pythonOutputDownload');if(!win)return;
 if(src&&img){img.src=src;$('#pythonOutputTitle').textContent=String(title||'Python output').slice(0,80);link.href=src;link.download=`zebjus-${mime==='image/jpeg'?'opencv.jpg':'plot.png'}`;link.hidden=false}
 win.hidden=false;
}
function initPythonOutputWindow(){const win=$('#pythonOutputWindow'),drag=$('#pythonOutputDrag');$('#pythonOutputClose')?.addEventListener('click',()=>{win.hidden=true});$('#pythonOutputOpen')?.addEventListener('click',()=>{if(!$('#pythonOutputImage')?.src)return pythonCameraNote('Run a camera or plot example first.');openPythonOutputWindow()});if(!drag||!win)return;drag.addEventListener('pointerdown',e=>{if(e.target.closest('button,a'))return;e.preventDefault();drag.setPointerCapture(e.pointerId);const box=win.getBoundingClientRect(),dx=e.clientX-box.left,dy=e.clientY-box.top;const move=ev=>{win.style.left=`${Math.max(0,Math.min(innerWidth-80,ev.clientX-dx))}px`;win.style.top=`${Math.max(0,Math.min(innerHeight-45,ev.clientY-dy))}px`};const end=()=>{drag.removeEventListener('pointermove',move);drag.removeEventListener('pointerup',end);drag.removeEventListener('pointercancel',end)};drag.addEventListener('pointermove',move);drag.addEventListener('pointerup',end,{once:true});drag.addEventListener('pointercancel',end,{once:true})})}
function pythonVisualImage(m){const mime=m.mimeType==='image/jpeg'?'image/jpeg':'image/png',b64=String(m.base64||'');if(!b64||b64.length>3000000){pythonTerminalWrite('[IMAGE] Output was empty or exceeded 2 MB. Reduce plot size.\n','warn');return}const src=`data:${mime};base64,${b64}`,img=$('#pythonPlotImage'),link=$('#pythonPlotDownload');if(img){img.src=src;img.hidden=false;img.closest('details').open=true}if(link){link.href=src;link.download=`zebjus-python-output.${mime==='image/jpeg'?'jpg':'png'}`;link.hidden=false}openPythonOutputWindow(src,m.title||'Python image / plot',mime)}
function cleanPythonTraceback(error){return String(error||'').split(/\n\s+at (?:new_error|https?:|wrapper|Object\.|Module\.|e\.port)/)[0].trim()}
function sendPythonSafeFrame(){if(!pythonRealRcActive)return;pythonRealRcActive=false;const channels=[1500,1500,1000,1500,1000,1000,1000,1000,1500,1000];Promise.resolve(pythonLastHardwareCommand).catch(()=>{}).then(()=>{if(window.zebjusSchool?.canControl?.())return window.zebjusSchool.commandDevice({type:'rc_frame',channels})}).catch(e=>pythonTerminalWrite(`[SAFE RC] ${e.message}\n`,'warn'))}
async function pythonWorkerRpc(worker,m,target){try{let value=null;if(m.method==='keyboardState')value=JSON.stringify([...pythonHeldKeys]);else if(m.method==='attitude'){value=JSON.stringify(target==='sim'?await pythonSimCommand('attitude_read',{}):{roll:state.telemetry.roll,pitch:state.telemetry.pitch,yaw:state.telemetry.yaw})}else if(m.method==='status'){value=JSON.stringify(target==='sim'?{ok:true,target:'simulator',flightMode:state.sim.flightMode,pid:state.pid,rc:simReceiverChannels()}:window.zebjusSchool?.getSelectedDevice?.()||{})}else if(m.method==='i2cScan'){if(target!=='real')throw new Error('Hardware I²C is unavailable while Python target is Simulator.');if(!window.zebjusSchool?.isKitActive?.())throw new Error('Connect the selected ZEBJUS kit first.');value=JSON.stringify(await window.zebjusSchool.i2cScan())}else if(m.method==='imuRead'){if(target!=='real')throw new Error('Hardware IMU is unavailable while Python target is Simulator.');if(!window.zebjusSchool?.isKitActive?.())throw new Error('Connect the selected ZEBJUS kit first.');const d=await window.zebjusSchool.imuRead();recordImuSample(d);value=JSON.stringify(d)}else if(m.method==='cameraFrame'){value=pythonCameraFrame()}else if(m.method==='handLandmarks'){if(target==='real'&&window.zebjusSchool?.getSelectedDevice?.()?.armed)throw new Error('Hand tracking is paused while the real kit is armed.');value=JSON.stringify(pythonHandData)}else if(m.method==='command'){let data={};try{data=JSON.parse(String(m.args?.payload||'{}'))}catch{}const type=String(m.args?.type||'');let result;if(target==='sim'){result=await pythonSimCommand(type,data);if(type==='rc_frame')window.zebjusSchool?.mirrorPythonRc?.(data.channels,'sim',result)}else{if(type==='rc_frame'&&Number(data.channels?.[4])>=1500)pythonRealRcActive=true;const hardwarePromise=window.zebjusSchool?.commandDevice?.({type,...data});pythonLastHardwareCommand=Promise.resolve(hardwarePromise).catch(()=>{});result=await hardwarePromise;if(type==='rc_frame')window.zebjusSchool?.mirrorPythonRc?.(data.channels,'real',result);if(type==='rc_frame'&&Number(data.channels?.[4])<1500)pythonRealRcActive=false;if(!result)throw new Error('Real-kit command bridge unavailable. Reconnect the selected kit.');if(type==='pid_get'&&result?.pid){state.pid=normalizePidShape(JSON.parse(JSON.stringify(result.pid)));renderPid();syncQuickPid();updatePidCoach?.()}else if(['pid_set','pid_defaults','rc_frame','calibration_set','calibration_defaults','level_calibrate','calibrate_level','motor_test','motor_order_test','motor_stop','esc_calibrate'].includes(type)){try{await pythonSimCommand(type,data)}catch(e){console.warn('[ZEBJUS] Python simulator mirror:',e)}}}value=JSON.stringify(target==='real'&&result&&typeof result==='object'?{...result,simulatorMirror:true}:result)}else throw new Error('Unknown Python bridge method: '+m.method);if(worker===pythonWorker)try{worker.postMessage({type:'rpc-result',id:m.id,ok:true,value})}catch{}}catch(e){if(worker===pythonWorker)try{worker.postMessage({type:'rpc-result',id:m.id,ok:false,error:String(e?.message||e)})}catch{}}}
function finishPythonWorker(worker,status='Finished'){if(worker&&pythonWorker!==worker)return;clearInterval(pythonCameraPump);pythonCameraPump=0;sendPythonSafeFrame();window.zebjusSchool?.setPythonControlMode?.(false);pythonRcProject=false;if(worker){try{worker.terminate()}catch{}}if(pythonWorker===worker)pythonWorker=null;pythonSetRunUi(false);$('#pyStatus').textContent=status;const term=$('#pythonTerminal');if(term)term.scrollTop=term.scrollHeight}
async function runPythonLab(){
 if(pythonRunning){notify('A Python program is already running. Press Stop first.','bad');return}
 pythonFiles[pythonActiveFile]=getPythonCode();savePythonProject();const code=getPythonCode();
 if(!pythonHardwarePreflight(code))return;
 clearPythonMarkers();pythonStderrBuffer='';pythonStopRequested=false;pythonRealRcActive=false;pythonLastHardwareCommand=Promise.resolve();pythonHasRun=true;pythonSetRunUi(true);
 const target=$('#pythonTarget')?.value||'sim',cameraNeeded=/\b(?:camera_frame|VideoCapture|hands)\s*\(/.test(code),handsNeeded=/\bhands\s*\(/.test(code);
 pythonRcProject=/\bdrone\.rc\s*\(|\brc_frame\b/.test(code);
 if(syncPythonCodeToSimulator(code))pythonTerminalWrite(`[SIM SYNC] PID constants from this file were applied to Tripod Simulator${target==='real'?' while the real kit remains the hardware target':''}.\n`,'warn');
 try{
  if(cameraNeeded){$('#pyStatus').textContent='Opening browser camera…';await startPythonCamera();if(handsNeeded&&!pythonHandTimer){await togglePythonHands();if(!pythonHandTimer)throw new Error($('#pythonCameraStatus')?.textContent||'MediaPipe hand tracking could not start')}}
 }catch(e){pythonTerminalWrite(`[CAMERA] ${e.message}\n`,'error');$('#pyStatus').textContent='Camera unavailable';pythonSetRunUi(false);return}
 if(pythonRcProject){window.zebjusSchool?.setPythonControlMode?.(true,target);if(!cameraNeeded)document.querySelector('.tab[data-tab="joystick"]')?.click()}
 const worker=new Worker(`./python-worker.js?v=18.3.46`);pythonWorker=worker;const runId=++pythonWorkerRunId;
 $('#pyStatus').textContent=`Starting ${pythonActiveFile} in isolated Worker…`;
 pythonTerminalWrite(`\n▶ ${pythonActiveFile} • ${target==='real'?'REAL KIT':'SIMULATOR'}\n`);
 worker.onmessage=async e=>{
  if(runId!==pythonWorkerRunId)return;const m=e.data||{};
  if(m.type==='stdout')pythonTerminalWrite(m.text||'');
  else if(m.type==='stderr')pythonStderrBuffer+=String(m.text||'');
  else if(m.type==='status')$('#pyStatus').textContent=m.text||'Python runtime';
  else if(m.type==='image')pythonVisualImage(m);
  else if(m.type==='rpc')await pythonWorkerRpc(worker,m,target);
  else if(m.type==='started')$('#pyStatus').textContent=`Running ${pythonActiveFile} • LIVE`;
  else if(m.type==='done'){
   if(pythonStderrBuffer.trim())pythonTerminalWrite(pythonStderrBuffer,'error');
   finishPythonWorker(worker,`Finished • ${pythonActiveFile}`)
  }else if(m.type==='error'){
   const err={message:cleanPythonTraceback(m.error||'Python run failed')},friendly=pythonFriendlyError(err,pythonStderrBuffer),raw=[pythonStderrBuffer.trim(),err.message.trim()].filter(Boolean).join('\n');
   if(raw)pythonTerminalWrite(raw+(raw.endsWith('\n')?'':'\n'),'error');
   if(friendly&&!raw.includes(friendly))pythonTerminalWrite(`ERR: ${friendly}\n`,'error');
   markPythonError(err);const errorPosition=$('#pyStatus')?.textContent||'Python error';finishPythonWorker(worker,errorPosition)
  }
 };
 worker.onerror=e=>{pythonTerminalWrite(`ERR: ${e.message||'Python Worker failed'}\n`,'error');finishPythonWorker(worker,'Python Worker error')};
 if(cameraNeeded)startPythonCameraPump(worker);
 worker.postMessage({type:'run',filename:pythonActiveFile,code,target,files:{...pythonFiles,[pythonActiveFile]:code}})
}
function stopPythonLab(){if(!pythonRunning||!pythonWorker){$('#pyStatus').textContent='Python idle';return}const w=pythonWorker;pythonWorkerRunId++;pythonTerminalWrite('[STOP] Program terminated immediately.\n');finishPythonWorker(w,'Stopped • Worker terminated')}
function initPythonWorkspaceResizers(){const tab=$('#tab-python'),col=$('#pythonColResizer'),row=$('#pythonRowResizer');if(!tab)return;const key='zebjus-python-layout-v1845';try{const d=JSON.parse(localStorage.getItem(key)||'null');if(d?.side)tab.style.setProperty('--py-side-w',`${clamp(+d.side,300,820)}px`);if(d?.terminal)tab.style.setProperty('--py-terminal-h',`${clamp(+d.terminal,240,760)}px`)}catch{}const save=()=>{try{localStorage.setItem(key,JSON.stringify({side:parseFloat(getComputedStyle(tab).getPropertyValue('--py-side-w'))||430,terminal:parseFloat(getComputedStyle(tab).getPropertyValue('--py-terminal-h'))||440}))}catch{}};const drag=(el,onMove)=>{if(!el)return;el.onpointerdown=e=>{e.preventDefault();el.classList.add('dragging');el.setPointerCapture?.(e.pointerId);const move=x=>onMove(x,e);const up=()=>{el.classList.remove('dragging');window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up);save();pythonMonaco?.layout?.();drawImuTools?.()};window.addEventListener('pointermove',move);window.addEventListener('pointerup',up,{once:true})}};if(col){drag(col,(e,start)=>{if(innerWidth<1150)return;const r=tab.getBoundingClientRect(),next=clamp(r.right-e.clientX-8,300,Math.min(820,r.width*.55));tab.style.setProperty('--py-side-w',`${next}px`);pythonMonaco?.layout?.();drawImuTools?.()});col.ondblclick=()=>{tab.style.setProperty('--py-side-w','430px');save();pythonMonaco?.layout?.()}}if(row){drag(row,(e,start)=>{const side=$('.python-side-stack'),r=side?.getBoundingClientRect();if(!r)return;const next=clamp(r.bottom-e.clientY,240,760);tab.style.setProperty('--py-terminal-h',`${next}px`);drawImuTools?.()});row.ondblclick=()=>{tab.style.setProperty('--py-terminal-h','440px');save();drawImuTools?.()}}}
function initPython(){
 initPythonWorkspaceResizers();initPythonOutputWindow();window.zebjusStopPythonForSafety=stopPythonLab;$('#pythonCameraStart')?.addEventListener('click',()=>startPythonCamera().catch(e=>pythonCameraNote(e.message)));$('#pythonCameraStop')?.addEventListener('click',stopPythonCamera);$('#pythonHandsToggle')?.addEventListener('click',togglePythonHands);document.querySelectorAll('.tab[data-tab]').forEach(tab=>tab.addEventListener('click',()=>{if(tab.dataset.tab!=='python'&&pythonCameraStream)stopPythonCamera()}));loadPythonProject();const fallback=$('#pythonEditor');if(fallback)fallback.value=pythonFiles[pythonActiveFile]||PY_DEFAULT;renderPythonFiles();pythonSetRunUi(false);
 $('#runPythonBtn').onclick=runPythonLab;$('#rerunPythonBtn').onclick=runPythonLab;$('#rerunPythonBtn').disabled=true;$('#stopPythonBtn').onclick=stopPythonLab;$('#copyTerminalBtn').onclick=copyPythonTerminal;$('#clearTerminalBtn').onclick=()=>$('#pythonTerminal').textContent='';$('#pythonNewFileBtn').onclick=newPythonFile;$('#pythonSaveFileBtn').onclick=()=>{pythonFiles[pythonActiveFile]=getPythonCode();savePythonProject();$('#pyStatus').textContent=`Saved ${pythonActiveFile}`};$('#pythonUndoFileBtn').onclick=undoPythonProject;$('#pythonRedoFileBtn').onclick=redoPythonProject;$('#pythonDeleteFileBtn').onclick=()=>deletePythonFile();
 $('#pythonExportBtn').onclick=exportPythonProject;$('#pythonImportBtn').onclick=()=>$('#pythonImportFile').click();$('#pythonImportFile').onchange=e=>{importPythonProject(e.target.files?.[0]);e.target.value=''};
 ...
 $('#imuCalibrateBtn').onclick=calibrateImuTeaching;$('#imuRecordBtn').onclick=()=>{toggleImuRecording();if(!imuRecording&&$('#imuCsvBtn'))$('#imuCsvBtn').disabled=!imuCsvRows.length};$('#imuCsvBtn').onclick=downloadImuCsv;setInterval(updateImuToolAge,250);window.addEventListener('resize',drawImuTools);window.addEventListener('keydown',e=>{if(!(pythonRcProject&&$('#tab-joystick')?.classList.contains('active'))&&!$('#tab-python')?.classList.contains('active')||!pythonRunning)return;pythonHeldKeys.add(e.key);if(!e.ctrlKey&&!e.metaKey&&!e.altKey&&['ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.key))e.preventDefault()},{passive:false});window.addEventListener('keyup',e=>pythonHeldKeys.delete(e.key));window.addEventListener('blur',()=>{pythonHeldKeys.clear();if(pythonRcProject&&pythonRunning)stopPythonLab()});document.addEventListener('visibilitychange',()=>{if(document.hidden&&pythonRcProject&&pythonRunning)stopPythonLab()});
 ensureMonaco().then(()=>{setPythonCode(pythonFiles[pythonActiveFile]||PY_DEFAULT);renderPythonFiles()})
}

/* Save/load/buttons */
function save(){try{localStorage.setItem('zebjusF450V18',JSON.stringify({guided:state.guided,step:state.step,parts:state.parts.filter(p=>!p.internal&&p.type!=='batteryStrap').map(p=>({type:p.type,slotId:p.slotId})),actions:[...state.doneActions],connections:state.connections,pid:state.pid,wireLayout,wireNodeTransforms,optionalWireNodes,sim:{batteryV:state.sim.batteryV,payloadG:state.sim.payloadG,cgX:state.sim.cgX,cgY:state.sim.cgY,wind:state.sim.wind,motorLag:state.sim.motorLag}}));$('#saveState').textContent='Saved';setTimeout(()=>$('#saveState').textContent='Ready',800)}catch(e){console.warn('[ZEBJUS] Save unavailable:',e);notify('Browser storage is unavailable in this embed/session.','bad')}}
function load(){try{const rawCurrent=localStorage.getItem('zebjusF450V18')||localStorage.getItem('zebjusF450V1751')||localStorage.getItem('zebjusF450V175')||localStorage.getItem('zebjusF450V173')||localStorage.getItem('zebjusF450V172')||localStorage.getItem('zebjusF450V171')||localStorage.getItem('zebjusF450V17')||localStorage.getItem('zebjusF450V16')||localStorage.getItem('zebjusF450V152')||localStorage.getItem('zebjusF450V151'),rawLegacy=localStorage.getItem('zebjusF450V121')||localStorage.getItem('zebjusF450V12')||localStorage.getItem('zebjusF450V10')||localStorage.getItem('zebjusF450V9'),d=JSON.parse(rawCurrent||rawLegacy||'null');if(!d)return;history.restoring=true;state.guided=d.guided??true;state.step=rawCurrent?(d.step||0):0;state.doneActions=new Set(d.actions||[]);state.connections=d.connections||[];state.pid=normalizePidShape(d.pid||state.pid);wireLayout={...wireDefaultLayout,...(d.wireLayout||{})};wireNodeTransforms=d.wireNodeTransforms||{};optionalWireNodes=d.optionalWireNodes||[];if(d.sim)Object.assign(state.sim,d.sim);(d.parts||[]).filter(p=>p.type!=='fcStandoff'&&p.type!=='batteryStrap').forEach(p=>{const s=(slots[p.type]||[]).find(x=>x.id===p.slotId);if(s)install(p.type,s,false)});if(!rawCurrent){const firstIncomplete=steps.findIndex((_,i)=>!stepDone(i));state.step=firstIncomplete<0?steps.length-1:firstIncomplete}render2D();renderPid();rebuild3DWires();rebuildSolder();syncWiringActionsToAssembly();setPowerVisual(wiringActionReady('xt60'),true);persistWireLayout();history.restoring=false}catch(e){console.warn(e)}}
function stopInteractiveRuntime(){
 if(state.sim.running)stopSimRuntime(true);else{destroyMotorAudio('sim');destroySimMotorBank()}
 if(wireMotorRun){wireMotorRun=false;destroyMotorAudio('wire');render2D?.()}else destroyMotorAudio('wire');
 heldKeys.clear()
}
function initRuntimeSafety(){
 document.addEventListener('visibilitychange',()=>{if(document.hidden){stopTransientTones();stopInteractiveRuntime()}});
 window.addEventListener('pagehide',()=>{stopTransientTones();stopInteractiveRuntime()});
...
function initButtons(){
 ...
 $('#autoWireBtn').onclick=()=>{wireRemember();historyPush();state.connections=referenceWires().map(([from,to],i)=>({from,to,new:true,id:`ref-${Date.now()}-${i}`}));render2D();rebuild3DWires();rebuildSolder();advanceGuidedStepIfReady('wiring');notify('Full correct reference wiring created and synchronized to Assembly.');setTimeout(()=>{state.connections.forEach(c=>c.new=false);render2D()},900)};$('#clearWireBtn').onclick=()=>{wireRemember();historyPush();state.connections=[];render2D();rebuild3DWires();rebuildSolder();renderAssemblyUI()};
 const batteryBtn=$('#batteryConnectBtn');if(batteryBtn)batteryBtn.onclick=toggleBatteryPower;const legacyConnect=$('#connectFcBtn');if(legacyConnect)legacyConnect.onclick=connectFc;const legacyDisconnect=$('#disconnectFcBtn');if(legacyDisconnect)legacyDisconnect.onclick=disconnectFc;const legacyPing=$('#pingFcBtn');if(legacyPing)legacyPing.onclick=()=>sendFc({type:'ping',time:Date.now()});const applyPidBtn=$('#applyPidBtn');if(applyPidBtn)applyPidBtn.onclick=()=>{syncQuickPid();notify('PID values applied to tripod simulator.');};const readPidBtn=$('#readPidBtn');if(readPidBtn)readPidBtn.onclick=()=>{updatePidSaveState('pending');if(!sendFc({type:'pid_get'}))markPidDirty()};const sendPidBtn=$('#sendPidBtn');if(sendPidBtn)sendPidBtn.onclick=()=>{updatePidSaveState('pending');if(!sendFc({type:'pid_set',...pidCommandPayload(state.pid)}))markPidDirty()};const restorePidBtn=$('#restorePidBtn');if(restorePidBtn)restorePidBtn.onclick=()=>{historyPush();state.pid=normalizePidShape({});markPidDirty();renderPid();syncQuickPid()};
...
window.zebjusLabAPI={
 ...
 receiveDevicePacket:d=>packet(d),
 setFcConnected:on=>fcStatus(!!on),
 logFc:t=>fcLog(String(t)),
 getActiveTab:()=>document.querySelector('.tab.active')?.dataset.tab||'assembly',
 setActiveTab:t=>{const b=document.querySelector(`.tab[data-tab="${t}"]`);if(b)b.click()},
 setViewOnly:on=>{window.__zebjusViewOnly=!!on;document.body.classList.toggle('student-view-only',!!on)},
 getProjectSnapshot:()=>projectPayload()
};
...
async function boot(){
 ...
 bootStep('Settings',initSettings);bootStep('Runtime safety',initRuntimeSafety);registerOffline();
 let assets={loaded:0,total:0,failed:0};
 if(threeOK){setBootStatus('Loading local component models…');assets=await loadAssets()}
 ...
 window.dispatchEvent(new CustomEvent('zebjus-app-ready',{detail:{three:threeOK,version:'18.3.46',assets}}));
}
